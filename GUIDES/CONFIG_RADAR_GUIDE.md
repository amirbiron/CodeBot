...
## כרטיסיות Quick Fixes ו-Runbooks (חדשות)

מתחת לטאבים של `alerts.yml`, `error_signatures.yml` ו-`image_settings.yaml` תמצא כעת שתי כרטיסיות נוספות:

- **Quick Fixes** – מציגה תקציר של `config/alert_quick_fixes.json`: כמה סוגי התראות מכוסים, אילו פעולות דוגמה (copy/link) מוכנות לכל alert type ומי מפעיל fallback ברירת מחדל. זה המקום לבדוק במהירות האם הוספת פקודה חדשה (למשל `/cache_clear_stale`) באמת זמינה לכל מי שלוחץ על הכרטיסיה בחיפוש הגלובלי.
- **Runbooks** – מושך נתונים מתוך `config/observability_runbooks.yml`, כולל ה-Runbook המוגדר כ-default, מספר הצעדים בכל תרחיש ושמות הצעדים (כולל action label). אם פלייבוק חסר או אם ברירת המחדל לא מוגדרת, הפאנל מסמן זאת כחוסר עקביות.

כך אפשר לראות במבט אחד האם כפתורי ה-Quick Fix וזרימות ה-Runbook תואמים לנתונים שבקוד – בלי לפתוח את הקבצים ידנית.
