...
class LazyLoader:
    ...
    def get_file_summary(
        self, code: str, file_name: str, programming_language: str
    ) -> Dict[str, any]:
    ...
    def _analyze_file_structure(self, code: str, language: str) -> Dict[str, any]:
        ...
        structure = {"functions": [], "classes": [], "imports": [], "main_sections": []}
            ...
            for i, line in enumerate(lines, 1):
                ...
                if language.lower() == "python":
                    if line_stripped.startswith("def ") or line_stripped.startswith("async def "):
                        ...
                        structure["classes"].append({"name": class_name, "line": i})

                    elif line_stripped.startswith("import ") or line_stripped.startswith("from "):
                        structure["imports"].append({"statement": line_stripped, "line": i})

                # זיהוי sections עיקריים (הערות גדולות, docstrings)
                if line_stripped.startswith('"""') or line_stripped.startswith("'''"):
                    if current_section is None:
                        current_section = {"start": i, "type": "docstring"}
                    else:
    ...
    def get_navigation_keyboard(self, chunk: FileChunk, user_id: int):
        ...
        if chunk.chunk_index > 0:
            nav_row.append(
                InlineKeyboardButton(
                    "⬅️ קודם", callback_data=f"chunk:{chunk.file_name}:{chunk.chunk_index - 1}"
                )
            )
        ...
        if chunk.chunk_index < chunk.total_chunks - 1:
            nav_row.append(
                InlineKeyboardButton(
                    "➡️ הבא", callback_data=f"chunk:{chunk.file_name}:{chunk.chunk_index + 1}"
                )
            )
    ...
    async def show_large_file_lazy(
        self, update, user_id: int, file_name: str, chunk_index: int = 0
    ):
        ...
        try:
            ...
            if not chunk:
                await update.message.reply_text(
                    f"❌ לא ניתן לטעון את הקובץ '{html_escape(file_name)}'", parse_mode="HTML"
                )
                return
            ...
            programming_language = (
                file_data.get("programming_language", "text") if file_data else "text"
            )
