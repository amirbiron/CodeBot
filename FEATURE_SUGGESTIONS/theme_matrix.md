...
| ``--ck-toast-*`` | Override כהה ב-`toast.css` | ברירת המחדל הבהירה | ברירת המחדל הבהירה | ברירת המחדל הבהירה | ברירת המחדל הבהירה | שחור/לבן ייעודי |
...
- **ווי דריסה (opt-in)** – ``--collections-link-override`` ו‑``--search-suggestion-text-override`` אינם בטבלה שלמעלה בכוונה: אין להם ערך בשום ערכה, ולכן אין להם "כיסוי" לבדוק. הם קיימים כדי שערכה מיובאת בודדת תוכל לדרוס צבע לעצמה, וכל כלל שצורך אותם נופל לערך שנצבע בו היום כשהערכה שותקת. הבדיקה כאן הפוכה: ודאו שהטוקן **לא** קיבל ערך ברירת מחדל בשום קובץ, אחרת הדריסה הפכה גורפת. ראו `docs/webapp/theming_and_css.rst` בסעיף החריגים.
