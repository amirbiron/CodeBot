...
> **סוכן שאין לו גישה לריפו — כמו Claude בחלון צ'אט — קורא דרך השרת הזה קבצים אמיתיים מהקוד.** זה הרבה יותר מלחסוך העתק-הדבק.

### למה זה שימושי

**לשאול על קוד בלי להרים סשן פיתוח.** ל-Claude בצ'אט אין ריפו, אין קלון ואין טרמינל. דרך ה-MCP הוא קורא את הקבצים מהמראות המקומיות, ולכן אפשר לחקור, לשאול ולנסח מול הקוד האמיתי בחלון צ'אט רגיל — במקום לפתוח סשן Claude Code שלם על כל שאלה קטנה.

**וגם בכיוון ההפוך:** אפשר להתייעץ בצ'אט על משהו ש-Claude Code כתב בסשן, כשהיועץ באמת **רואה את הקוד** ולא קטע שהודבק לו בלי הקשר. וזה לא מוגבל לריפו אחד — כל ריפו שמתווסף למראות נקרא באותה צורה בדיוק.

**Claude Code שולף כלל אחד במקום לטעון ספר.** ב-`CLAUDE.md` של הפרויקט יש טבלת טריגרים שמפנה לסעיפים בריפו נפרד של דפוסי באגים. כשטריגר נדלק — נגיעה בקאש, `try/except` סביב קריאה, חיתוך טקסט עברי — הסוכן מביא **את הסעיף הזה בלבד** דרך `docs_get_section`, לא את הקובץ ולא את הריפו. הכללים נשארים מחוץ לחלון ההקשר עד הרגע שבו הם רלוונטיים, וזה מה שהופך אותם לשימושיים במקום לכבדים.

**והפריימר: הוראות שנטענות לבד.** בעמוד ההגדרות של הוובאפ יש בלוק טקסט חופשי, והוק `SessionStart` מביא אותו **בפתיחת כל סשן וגם בכל resume**. העדפות עבודה, אילוצי סביבה והקשר על הפרויקטים מגיעים לסוכן בלי להעתיק אותם ידנית בכל פעם — גם אחרי שסשן ארוך התחיל מחדש.

> **סייג:** קריאה חופשית מהמראות (`get_repo_file`, `search_repo`, `list_repo_tree`) היא **פיצ'ר אדמין** כרגע. החריג הוא `docs_get_section`, שפתוח לכל משתמש מחובר — והוא זה שמאפשר את קריאת דפוסי הבאגים שלמעלה.
