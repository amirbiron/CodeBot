...
│   ├── note_boards_api.py           # API לוחות פתקים
...
### 7️⃣ פתקים דביקים ולוחות פתקים

**מערכת פתקים מקיפה — על קבצים או על לוחות עצמאיים:**

#### פתקים דביקים (Sticky Notes)

פתק יכול להיות מוצמד לקובץ או לחיות על לוח פתקים עצמאי.

**תכונות משותפות:**
- מיקום, גודל וצבע מותאמים אישית
- קיפול/פתיחה (`is_minimized`)
- תזכורות עם snooze (6 presets + זמן מותאם אישית)
- קישור קבוע `/note/<note_id>` — מפנה לקובץ או ללוח
- תקרות: עד 200 פתקים ללוח/קובץ, עד 1000 פתקים למשתמש, עד 5,000 תווים לפתק
- משתמשי אדמין פטורים מתקרות

**פתקי קבצים:**
- עיגון לעוגני כותרות Markdown (`anchor_id`, `anchor_text`)
- עיגון לשורת מקור (`line_start`, `line_end`)
- מצב "עוגן" (המיקום נגזר מהעוגן)

**פתקי לוח:**
- שני מצבי מיקום:
  - `surface` (צמוד ללוח, זז איתו)
  - `screen` (צמוד למסך, נשאר במקום גם כשגוללים)
- חופש מלא במיקום — אין עוגנים, הפתק לא נעלם אם הקובץ משתנה

**צ'קבוקסים אינטראקטיביים:**
- שורות בפורמט `- [ ] text` מוצגות כתיבות סימון לחיצות
- לחיצה שומרת מיד למסד, עם אימות מלא (compare-and-swap + קריאה חוזרת)
- זיהוי לפי סדר הופעה, כך שאפשר לסמן כל סימון בנפרד

#### לוחות פתקים (Note Boards)

משטחים לארגון פתקים שאינם שייכים לשום קובץ.

**ניהול לוחות:**
- יצירה, שינוי שם, מחיקה (`/api/note-boards`)
- לוח ברירת מחדל "לוח עבודה" (`is_default: true`) נוצר אוטומטית
- **לוח ברירת המחדל לא ניתן למחיקה** — הוא היעד האוטומטי לפתקים יתומים
- מחיקת לוח — הפתקים עוברים אוטומטית ללוח ברירת המחדל (ספירה לפני ואחרי, כשל עוצר את המחיקה)

**תקרות:**
- עד 200 פתקים ללוח
- עד 500 לוחות למשתמש (בפועל אין מגבלה בשימוש רגיל)

**ריפוי פתקים יתומים:**
- אם פתק מצביע ללוח שנמחק, הוא מחובר מחדש ללוח ברירת המחדל (`reattach_orphan_notes`)
- רץ אוטומטית בטעינת הדף

**API Endpoints:**
- `GET /api/note-boards` — רשימת לוחות + ספירת פתקים
- `POST /api/note-boards` — לוח חדש
- `PATCH /api/note-boards/<id>` — שינוי שם
- `DELETE /api/note-boards/<id>` — מחיקה (אחרי העברת פתקים)
- `GET /api/sticky-notes/board/<board_id>` — פתקי הלוח
- `POST /api/sticky-notes/board/<board_id>` — פתק חדש על הלוח
- `POST /api/sticky-notes/note/<note_id>/task` — toggle צ'קבוקס

**UI Pages:**
- `/note-boards` — רשימת הלוחות
- `/note-boards/<id>` — תצוגת לוח ספציפי
- `/note/<note_id>` — קישור קבוע (מפנה לקובץ או ללוח לפי הצורך)

---

### 8️⃣ ספריית סניפטים (`/snippets`, `/api/snippets/`)
...
### 9️⃣ ספריית קהילה (`/community-library`, `/api/community-library/`)
...
### 🔟 לוח בקרה וסטטיסטיקות (`/dashboard`)
...
- **מידע מוצג**:
  - כותרת הפתק
  - שם הקובץ או הלוח המקושר
  - זמן יחסי (עודכן לפני X)
  - קישור לקובץ או ללוח המקור
...
- **פתקים דביקים (Sticky Notes)** — הערות צפות מעל תצוגת ה-Markdown או על לוחות פתקים עצמאיים, עם מיקום/גודל/צבע מותאמים. פתקי קבצים מתעגנים לעוגני כותרות Markdown, פתקי לוח יכולים להיות צמודי-משטח (זז עם הלוח) או צמודי-מסך (נשארים במקום).
  - **תזכורות לפתקים** — 6 presets מוכנים + זמן מותאם אישית + snooze, עם תמיכה באזורי זמן. עובדות גם על פתקי קבצים וגם על פתקי לוח.
  - 📎 לפירוט מלא (API, לוחות, תזכורות, סכימת DB): ראו סעיף WebApp **פתקים דביקים ולוחות פתקים**.
...
- `sticky_notes` - פתקים מהירים (על קבצים וללוחות)
- `note_boards` - לוחות פתקים
