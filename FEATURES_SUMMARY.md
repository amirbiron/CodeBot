...
4. [פיצ'רים של ה-Web App](#-פיצרים-של-ה-web-app)
   ...
   - 30. [אייקוני שפות התכנות](#3️⃣0️⃣-אייקוני-שפות-התכנות)
...
- חסימת משתמשים בשער הכניסה (שתי שכבות: ENV + DB)
...
3. **תצוגה מקדימה מרונדרת** (`/md/`, `/html/`, `/raw_html/`, `/svg/`, `/raw_svg/`)
   - 📝 **Markdown וHTML**: כל היכולות (רינדור עשיר, Mermaid, KaTeX, פתקים, ניווט כותרות, iframe בטוח) מרוכזות בסעיף **[📝 מסמכי Markdown — מרכז מאוחד](#-מסמכי-markdown--מרכז-מאוחד)**.
   - 🖼️ **SVG**: קבצי `.svg` מקבלים כפתור 🌐 (כמו HTML ו-Markdown), והלחיצה פותחת תצוגה מרונדרת עם החלפת רקע (כהה/בהיר/משבצות שקיפות), מסך מלא, פתיחה בחלון חדש, וקישורים לקוד ולהורדה.
     - `/svg/<id>` — עמוד התצוגה עם בקרות רקע ומסך מלא.
     - `/raw_svg/<id>` — ה-SVG הגולמי (`image/svg+xml`) עם CSP (`sandbox`, `script-src 'none'`) והשלמת `xmlns` אוטומטית.
     - **זיהוי לפי סיומת** `.svg` (לא לפי השפה `xml`) — להבדיל בין SVG לבין XML רגיל.
     - **אזהרת ספרייט**: קבצים עם `<symbol>` מציגים הודעה שהתצוגה עלולה להיראות ריקה עד שמפנים אליהם ב-`<use>`.
...
### 3️⃣0️⃣ אייקוני שפות התכנות

**28 אייקונים מצוירים במקום אמוג'ים, מספרייט אחד.**

עד אוגוסט 2026 כל קובץ הוצג עם אמוג'י לפי שפתו (🐍 לפייתון, 📜 ל-JavaScript). היום אלה אייקונים מצוירים (SVG) בסגנון אחיד — אריח ריבועי עם גרדיאנט וסימן לבן.

#### למה זה נבנה כמערכת ולא כהחלפת תמונות

לפני השינוי, אייקוני השפה יוצרו ב**חמישה מקומות נפרדים** עם כיסוי שונה: `get_language_icon` בשרת כיסה 28 שפות, `getFileIcon` בצד הלקוח כיסה 8, ועוד מפות חלקיות ב-`global_search.js`, `snippets.js` ו-`collections.js`. התוצאה: אותו קובץ קיבל אייקון אחד בדף הקבצים ואייקון אחר בחיפוש הגלובלי.

לכן המערכת נבנתה סביב מקור אמת אחד:

- **ספרייט** ב-`webapp/templates/components/lang_sprite.html`, מוזרק פעם אחת ב-`base.html`.
- **`lang_icon()`** ב-`webapp/app.py` — גלובל של Jinja, זמין בכל תבנית.
- **`window.langIcon()` / `window.langIconEl()`** ב-`base.html` — הצד הלקוחי, שמקבל את מפת השפות מהשרת דרך `window.LANG_ICON_DATA` ולכן אינו מחזיק מפה משלו.
- **`scripts/build_lang_sprite.py`** גוזר את הספרייט מ-28 קבצי המקור. הספרייט אינו נערך ידנית.

#### גדלים

מרוכזים ב-`LANG_ICON_SIZES` ונחשפים גם ל-Jinja וגם ללקוח:

| שם | גודל | היכן |
| --- | --- | --- |
| `file_row` | 44px | דף הקבצים, כרטיסים נעוצים, עמוד הקובץ |
| `list` | 32px | דשבורד, קבצים משותפים, סל מיחזור, מודאלים |
| `timeline` | 28px | אירועי קבצים בטיימליין |
| `search` | 28px | תוצאות החיפוש הגלובלי |
| `compact` | 22px | ספריית הסניפטים |

**למה 44px ולא 32px כמו האמוג'י שהוחלף:** מדידה בדפדפן הראתה שאמוג'י ב-`font-size: 32px` מצויר בפועל ב-35.5px (הגליף חורג מהמסגרת), בעוד אייקון SVG ב-32px מצויר ב-30px. וחשוב מכך — הסימן הלבן בתוך אריח של 32px נמדד 15×15 פיקסלים, מול צורת אמוג'י שממלאת 35.5px. הסימן שהעין מזהה קטן פי 2.4. זו תכונה של האריח, שיש לו נשימה מכוונת סביב הסימן, ולכן החלפה 1:1 בגודל תיראה תמיד קטנה יותר.

#### התנהגות

- שפה עם אייקון מצויר — מקבלת אותו.
- שפה בלי אייקון אבל עם סמל ייחודי (למשל 🔒 לקובץ בינארי) — שומרת על הסמל ולא מאבדת מידע.
- שפה שאינה מוכרת — מקבלת את אייקון ה-`text`, כדי שלא תתקבל תערובת של אמוג'י ואייקונים באותה רשימה.
- `LANG_ICON_ALIASES` ממפה שפות שחולקות אייקון: `shell`/`sh`/`zsh` ל-`bash`, `scss`/`sass`/`less` ל-`css`, `node` ל-`javascript` ועוד.
- האייקון מוסתר מקוראי מסך כברירת מחדל, כי לצדו תמיד מופיע שם הקובץ או תגית השפה.

#### לסוכני AI

שדה `icon` בתגובות ה-JSON עדיין מחזיר אמוג'י, לתאימות לאחור — אך הוא **אינו** מה שמוצג למשתמש. לזיהוי שפה יש להסתמך על השדה `language` או `programming_language`.

> **תיעוד מלא:** [אייקוני שפות התכנות](https://amirbiron.github.io/CodeBot/webapp/language-icons.html) — כולל אופן הוספת אייקון, מבנה קובץ SVG, והמלכודות שנתקלנו בהן.
