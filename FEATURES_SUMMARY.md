...
> **סוכן שאין לו גישה לריפו — כמו Claude בחלון צ'אט — קורא דרך השרת הזה קבצים מהקוד.**

### למה זה שימושי

**לשאול על קוד בלי להרים סשן פיתוח.** ל-Claude בצ'אט אין ריפו, אין קלון ואין טרמינל. דרך ה-MCP הוא קורא את הקבצים מהמראות המקומיות, ולכן אפשר לחקור, לשאול ולנסח מול הקוד האמיתי בחלון צ'אט רגיל — במקום לפתוח סשן Claude Code שלם על כל דבר.

**וגם בכיוון ההפוך:** אפשר להתייעץ בצ'אט על משהו ש-Claude Code כתב בסשן, כשהיועץ באמת **רואה את הקוד** ולא קטע חלקי מהקוד שהודבק לו בלי הקשר. וזה לא מוגבל לריפו אחד — כל ריפו שמתווסף למראות נקרא באותה צורה בדיוק.

**Claude Code שולף כלל אחד במקום לטעון ספר.** ב-`CLAUDE.md` של הפרויקט יש טבלת הפניות שמפנה לסעיפים בריפו נפרד של דפוסי באגים. כשטריגר נדלק Claude Code משתמש בכלי `docs_get_section` ומושך את השורות הספציפיות שהופנה אליהן מהריפו הנוסף.

מה הרווחים כאן?

1. הבעיה היא שאם נעתיק המון שורות קוד מהריפו הנוסף ל-`CLAUDE.md` של הפרויקט — קלאוד פשוט לא יציית להכל, מריבוי הנחיות. יוצא הפסד. מה גם שאין אפשרות לתמצת את הריפו הנוסף לתוך קובץ `CLAUDE.md` אחד.

**והפריימר: הוראות שנטענות לבד.** בעמוד ההגדרות של הוובאפ יש בלוק טקסט חופשי, והוק `SessionStart` מביא אותו ל-Claude Code **בפתיחת כל סשן וגם בכל resume**.

מה התועלת בזה?

1. אמנם `CLAUDE.md` גם נטען לקלאוד בכל תחילת סשן, אבל בפריימר אפשר להוסיף נקודות שפחות נוח לשים בתוך הריפו עצמו, שהוא גלוי לעוד אנשים.
2. דבר יעיל שמצאתי — להדגיש בפריימר כל מיני נקודות שכבר נמצאות ב-`CLAUDE.md`, מה שבאמת חשוב לכם, וקריטי שקלאוד לא ידלג עליו.
3. ההוק עובד חלק ומיידי: פתיחת סשן כמה שניות לאחר עדכון הפריימר מזריקה לקלאוד את התוכן המעודכן — מאומת מניסיון. (הפריימר מוגש עם cache של דקה לכל משתמש, אבל בין סשן לסשן עוברת בדרך כלל יותר מדקה, ולכן בפועל הוא כמעט תמיד קר.)
...
כלי משתמש לכל מי שמחובר, וכלי ריפו שפתוחים לאדמין בלבד — אלה שגופם פותח ב-`require_admin`. הספירה יושבת בסעיף [סטטיסטיקות פרויקט](#-סטטיסטיקות-פרויקט), כדי שלא תהיה בשני מקומות.
...
התועלת מתוארת למעלה; כאן החוזה. **`GET /api/agent/primer`** הוא אנדפוינט HTTP רגיל ולא כלי MCP, והוא **חי על שירות ה-MCP ולא על הוובאפ**.
...
זו הנקודה הארכיטקטונית המרכזית. ברירת המחדל של CPython ל-`ThreadPoolExecutor` היא `min(32, os.cpu_count() + 4)` — ו-`os.cpu_count()` **בקונטיינר מדווח את ליבות המארח**, כפי ש-CPython עצמו מתעד. כך זה נראה בייצור **לפני שהמאגר הוחלף**, כשעוד רצה ברירת המחדל:
...
שנים-עשר חוטים על חצי מעבד — **זה המצב הישן, לא הנוכחי.** לקריאת מונגו זה לא מזיק — החוטים ישנים; אבל **לפרסור בפייתון טהור אין מקביליות בגלל ה-GIL**, ולכן מאגר רחב קונה רק מקביליות בזיכרון: שנים-עשר עצי פרסור בו-זמנית, בלי תפוקה. **החסם האמיתי הוא הזיכרון**, וממנו נגזר הגודל:
...
**והתוצאה היום, על תוכנית הייצור (0.5 CPU / 512MiB): עשרה חוטים** — ‏`(512 − 92 − 64) ÷ 35.2 = 10`. ‏`_read_pool_size` ב-`mcp_server/server.py` היא הפונקציה הטהורה שעושה את החשבון, וכל אחד מהתנאים מעוגן בטסט מול מספר שחושב ביד — כך שמימוש שמחזיר קבוע נופל.

> **ושנים-עשר לא נעלם — הוא הפך לתקרה.** ‏`_READ_POOL_CAP` הוא בדיוק הרוחב שהייצור כבר הריץ בשלום לפני השינוי, וזו הסיבה שהוא נבחר: מעבר לתוכנית גדולה יותר לא ירחיב את המאגר מעבר לרוחב שכבר הוכח, בלי שמישהו יחליט על כך.
...
> **זה המקום היחיד במסמך שנושא מספרי קנה-מידה.** עץ התיקיות מתאר מבנה בלבד, כדי שלא יהיו שתי רשימות שצריך לעדכן יחד. המספרים נמדדו ב-21.09.2026 מתוך הקבצים שבמעקב git. מדדי הקוד מחריגים את `tests/` ואת הקוד החיצוני; ספירת קובצי הבדיקה היא מדידה נפרדת, ולכן היא מופיעה בשורה משלה.

### גודל Codebase
- **~182,000 שורות** קוד Python ב-**291 מודולים** — **בלי** `tests/`
- **868 קבצי בדיקה** תחת `tests/`, ספירה נפרדת שאינה נכללת בשתי השורות שמעליה
...
- **30 כלי MCP** שהשרת רושם בעצמו — 23 למשתמש, 7 לאדמין. סוכן רואה **31**, כי ספריית המדידה מוסיפה כלי וירטואלי אחד (`get_more_tools`) כשהיא מופעלת
