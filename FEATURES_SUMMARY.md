...
- **מגבלות**: 200 פתקים ללוח, 1,000 פתקים למשתמש, 20,000 תווים לפתק. חריגה באורך **נדחית ב-`content_too_long` ואינה נחתכת בשקט**, והממשק מציג מונה תווים אדום כבר בזמן ההקלדה.
...
הכפתור `🎨` פותח בורר עם שבעה צבעים: **צהוב, צהוב בהיר, ירוק בהיר, כתום בהיר, כחול בהיר, סגול בהיר וורוד בהיר**. הצהוב הראשון הוא ברירת המחדל, וגם הצבע שהפתקים תמיד היו בו.
...
| רשימות | `-` / `*` / `1.`, כולל **מקוננות עד חמש רמות** |
...
| בלוקי אלרט | 14 סוגים: `note`, `tip`, `warning`, `danger`, `important`, `info`, `success`, `question`, `example`, `quote`, `experimental`, `deprecated`, `todo`, `abstract` |
...
- **בלוק קוד בעברית בלי שם שפה מיושר לימין** (מעל 30% אותיות עבריות), כי בלוק כזה הוא כמעט תמיד הערה ולא קוד. בלוק שנכתב עם שם שפה (```` ```python ````) נשאר מיושר לשמאל גם אם התוכן עברי.
...
- כל תוצאה מציגה שם, **200 תווים ראשונים** עם המילה מודגשת, ואיפה הפתק יושב. אם המילה יושבת אחרי 200 התווים — כתוב "המילה מופיעה בהמשך הפתק", כדי שלא תתהו למה הפתק ברשימה.
...
- **קביעת תזכורת**: אייקון `🔔` בתוך הפתק פותח חלון תזמון עם קיצורי זמן (עוד שעה / 3 שעות / היום 21:00 / מחר 9:00 / 24 שעות / שבוע) או בחירה ידנית ב-`datetime-local`.
- **התראת Web Push** לנייד ולדפדפן כשמגיע הזמן, עם כפתורי דחייה. **דחייה גוברת על אישור-צפייה** — תזכורת שנדחתה חוזרת במועד שנבחר.
- **בועת התראה עם מונה** בסרגל העליון (במובייל מעל תפריט ההמבורגר, בדסקטופ מעל קישור ההגדרות). לחיצה פותחת **Pop-up עם שורה קצרה מכל פתק ממתין** (עד 6 מילים) ולא מנווטת מיד.
...
- **בדשבורד**: כרטיס **"פתקים אחרונים"** (עד 10 פתקים לפי תאריך עדכון, עם שם הקובץ, זמן יחסי וקישור), וכרטיס **הגדרות פוש** עם מספר מנויים פעילים, תזכורות ממתינות, הפוש האחרון שנשלח והתזכורת הבאה המתוזמנת.
...
**23 כלי משתמש** (לכל משתמש מחובר) ו-**7 כלי אדמין** (דפדפן הריפו).
...
הכלי שהופך ריפו של תיעוד ל-API. במקום למשוך קובץ בן 2,500 שורות כדי לקרוא סעיף אחד:
...
**פייתון** — שמות מלאים מנוקדים (`ClassName.method`, `outer.inner`). הכלל: **מרחב שמות הוא פונקציה או מחלקה בלבד** — `if`, `try`, `with` ו-`for` אינם, ולכן פונקציה שהוגדרה בתוך `except ImportError` מופיעה ברמה שלה. הכלל הזה הוא כל הפיצ'ר: בלעדיו `mcp_server/server.py` החזיר תשעה סימבולים במקום 38. ו**שורת ההתחלה היא של המעטר** — טווח שמתחיל ב-`def` היה מחמיץ את `@app.route(...)`, כלומר בדיוק את השורה שמזהה את הנתיב.
...
- **הפונקציות בתוך בלוק סקריפט הן העיקר.** בלוק בן מאות שורות שמוחזר כסימבול אחד נותן גבול ולא ניווט — ב-`base.html` הבלוק הגדול ביותר עובר שבע-מאות שורות ומפורק לעשרות הגדרות.
...
| תקרת 24KB | חריגה גורמת לחיתוך + שורה שמודיעה על כך, **ולעולם לא לשגיאה** — פריימר חתוך עדיף על פריימר חסר, ולשורת המצב שמור מקום שאינו נדחק החוצה |
| Cache | 60 שניות לכל משתמש |
| סינון סודות | כל הגוף עובר צנזור של דפוסים שנראים כמו מפתח (`ghp_`, `sk-`, JWT, `KEY=...`) לפני ההחזרה — גם אם הודבקו לשדה בטעות |

- **האחסון (48KB) רחב מההגשה (24KB) בכוונה**: החיתוך הוא החלטה של ההגשה ולא של האחסון, כדי שלא יימחק טקסט שהמשתמש כתב. מאותה סיבה הסודות נשמרים כפי שהם ומסוננים רק בהגשה — אחרת המשתמש היה פותח את ההגדרות ומוצא `***REDACTED***` בלי לדעת מה נמחק לו.
...
- **עקיפת הכתובת:** `CODEKEEPER_PRIMER_URL`. וה-timeout בהוק נדיב (20 שניות), כי השירות רץ על Render ויכול להיות "ישן" — התעוררות לבדה אוכלת כמה שניות.
...
- **הגבלת קצב לפי זהות** — 60 קריאות בדקה, בנקודת השיגור של הכלים ולא במידלוור. זו הנקודה היחידה שרואה את הזהות **בשני מצבי האימות**, וההכרעה קורית **לפני** שהקריאה תופסת חוט מהמאגר. המספר נגזר ממדידה: המכסה היא 0.5 מעבד, כלומר 30 שניות-מעבד בדקה, ושישים קריאות של המסמך הצפוף ביותר בפועל הן 28 שניות — זהות אחת יכולה למלא לכל היותר דקת מעבד אחת משלה, ולא לחנוק את השאר.
...
זו הנקודה הארכיטקטונית המרכזית. ברירת המחדל של CPython היא `min(32, os.cpu_count() + 4)` — ו-`os.cpu_count()` **בקונטיינר מדווח את ליבות המארח**. בייצור זה נראה כך:
...
- **קו הבסיס והשוליים נמדדו בייצור**, לא נבחרו: 92MiB RSS בשקט, ועוד 64MiB לכל מה שהתהליך מחזיק ואינו הפרסור המתוקצב.
- **עלות הפרסור נגזרת מצפיפות הטוקנים של המסמך ולא מגודלו**, ונמדדת מחדש בסקריפט ייעודי כשהפרסר משתנה.
- **רצפה (2)**, כדי שקריאת מונגו תקועה אחת לא תשבית את השאר, ו**תקרה (12)**, כדי שתוכנית גדולה יותר לא תרחיב את המאגר בלי שאיש החליט. וכשאין מגבלה שאפשר לקרוא — הגודל הוא **הרצפה**, ולא `cpu_count + 4`: מאגר צר מדי הוא איטיות גלויה, מאגר רחב מדי הוא OOM שאיש לא מייחס.

**על תוכנית הייצור (0.5 CPU / 512MiB):** ‏`(512 − 92 − 64) ÷ 35.2 = 10` חוטים, במקום 12.

#### המעבר לפרנקפורט — פי 45 על שמירה

השירות רץ תחילה ב-Render באורגון, בזמן שמסד הנתונים ישב בפרנקפורט. הזמנים נראו כמו בעיית קוד:

| מדד (p50) | אורגון | פרנקפורט | שיפור |
| --- | --- | --- | --- |
| `save_file` | 4,854ms | **108ms** | **פי 45** |
| `edit_file` | 4,392ms | **80ms** | **פי 55** |

**רוב הזמן הלך על הרשת בין Render למונגו**, לא על עיבוד. העברת השירות לאזור שבו כבר יושב המסד הורידה שמירה מכמעט חמש שניות למאית שנייה — בלי לשנות שורת קוד אחת בלוגיקה.
