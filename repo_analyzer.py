...
class RepoAnalyzer:
                    ...
                    if file_name.upper() in ["README.MD", "README.RST", "README.TXT", "README"]:
                        ...
                        try:
                            if file_content.size < 50000:  # מקסימום 50KB
                                readme_content = base64.b64decode(file_content.content).decode(
                                    "utf-8"
                                )
                        ...
                        if file_content.size > 0:
                            try:
                                ...
                                if file_content.size < self.MAX_FILE_SIZE:
                                    code_content = base64.b64decode(file_content.content).decode(
                                        "utf-8"
                                    )
                    ...
                    if file_name.lower() in self.CONFIG_FILES:
                        try:
                            if file_content.size < 50000:
                                config_content = base64.b64decode(file_content.content).decode(
                                    "utf-8"
                                )
                ...
                for line in content.split("\n"):
                    if "[tool.poetry.dependencies]" in line or "[project.dependencies]" in line:
                        ...
                        if len(parts) == 2:
                            ...
                            if name != "python":
                                dependencies.append(
                                    {"name": name, "type": "python", "version": version}
                                )
    ...
    def generate_improvement_suggestions(
        ...
        if main_language == "python":
            if not any(
                "black" in d.get("name", "")
                or "pylint" in d.get("name", "")
                or "flake8" in d.get("name", "")
                for d in deps
            ):
                suggestions.append(
                    {
            ...
            if not any(
                "eslint" in d.get("name", "") or "prettier" in d.get("name", "") for d in deps
            ):
                suggestions.append(
                    {
                        "id": "add_linter_js",
