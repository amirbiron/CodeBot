...
class RepoAnalyzer:
                        ...
                        try:
                            if file_content.size < 50000:  # מקסימום 50KB
                                ...
                                if len(readme_content) > 500:
                                    if any(
                                        section in readme_content.lower()
                                        for section in ["installation", "usage", "example", "התקנה", "שימוש", "דוגמה"]
                                    ):
                                ...
                                if file_content.size < self.MAX_FILE_SIZE:
                                    ...
                                    if line_count > self.LARGE_FILE_LINES:
                                        analysis["large_files"].append(
                                            {"path": file_path, "lines": line_count, "size": file_content.size}
                                        )
                ...
                for i, line in enumerate(lines):
                    ...
                    if match:
                        ...
                        if func_lines > self.LONG_FUNCTION_LINES:
                            ...
                            long_functions.append(
                                {"name": func_name, "type": "function", "lines": func_lines, "line_number": i + 1}
                            )
                ...
                for dep_type in ["dependencies", "devDependencies"]:
                    if dep_type in data:
                        for name, version in data[dep_type].items():
                            dependencies.append(
                                {"name": name, "type": "npm", "version": version, "dev": dep_type == "devDependencies"}
                            )
                ...
                for line in content.split("\n"):
                    if "[tool.poetry.dependencies]" in line or "[project.dependencies]" in line:
                        ...
                        if len(parts) == 2:
                            ...
                            if name != "python":
                                dependencies.append({"name": name, "type": "python", "version": version})
    ...
    def generate_improvement_suggestions(self, analysis_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if main_language == "python":
            if not any(
                "black" in d.get("name", "") or "pylint" in d.get("name", "") or "flake8" in d.get("name", "")
                for d in deps
            ):
                suggestions.append(
                    {
            ...
            if not any("eslint" in d.get("name", "") or "prettier" in d.get("name", "") for d in deps):
                suggestions.append(
                    {
                        "id": "add_linter_js",
