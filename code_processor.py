...
class CodeProcessor:
    ...
    def __init__(self):
        ...
        if not self.code_logger.handlers:
            ...
            )  # שימוש ב-StreamHandler במקום FileHandler לסביבת פרודקשן
        ...
        try:
            ...
            if "```" in text:
                ...
                def replace_code_block(match):
                    ...
                    # רישום הפעולה
                    self.code_logger.info(
                        f"מעבד בלוק קוד: {language_hint}, אורך: {len(cleaned_code)}"
                    )
                ...
                if processed_text == text:
                    ...
                    processed_text = re.sub(
                        simple_pattern, lambda m: m.group(1).strip(), text, flags=re.DOTALL
                    )
    ...
    def validate_code_input(
        self, code: str, filename: str = None, user_id: int = None
    ) -> Tuple[bool, str, str]:
        """
        ...
        try:
            ...
            if len(cleaned_code.strip()) == 0:
                if user_id:
                    ...
                    code_error_logger.log_validation_failure(
                        user_id, original_length, "הקוד ריק לאחר עיבוד"
                    )
            ...
            if len(cleaned_code) > 50000:  # 50KB limit
                if user_id:
                    ...
                    code_error_logger.log_validation_failure(
                        user_id, cleaned_length, "הקוד ארוך מדי (מעל 50KB)"
                    )
            ...
            try:
                ...
                if user_id:
                    ...
                    code_error_logger.log_validation_failure(
                        user_id, cleaned_length, "הקוד מכיל תווים לא חוקיים"
                    )
    ...
    def detect_language(self, code: str, filename: str = None) -> str:
        ...
        try:
            ...
            self.code_logger.info(
                f"קוד סונטז לזיהוי שפה, אורך מקורי: {len(code)}, אורך מנוקה: {len(sanitized_code)}"
            )
        except Exception as e:
    ...
    def highlight_code(
        self, code: str, programming_language: str, output_format: str = "html"
    ) -> str:
        ...
        try:
            ...
            if output_format == "html":
                formatter = HtmlFormatter(
                    style=self.style, noclasses=True, nowrap=True, linenos=False
                )
    ...
    def create_code_image(
        self, code: str, programming_language: str, width: int = 800, font_size: int = 12
    ) -> bytes:
    ...
    def get_code_stats(self, code: str) -> Dict[str, Any]:
        ...
        stats["functions"] = len(
            re.findall(r"\bdef\s+\w+\s*\(|\bfunction\s+\w+\s*\(", code, re.IGNORECASE)
        )
        ...
        logger.info(
            f"חושבו סטטיסטיקות לקוד: {stats['total_lines']} שורות, {stats['characters']} תווים"
        )
        return stats
    ...
    def extract_functions(self, code: str, programming_language: str) -> List[Dict[str, Any]]:
        ...
        if programming_language in patterns:
            ...
            for match in matches:
                ...
                functions.append(
                    {"name": func_name, "line": line_number, "signature": match.group(0)}
                )
        ...
        if programming_language == "python":
            try:
                ...
                result["errors"].append(
                    {"line": e.lineno, "message": str(e), "type": "SyntaxError"}
                )
            ...
            try:
                ...
                result["is_valid"] = False
                result["errors"].append(
                    {"line": e.lineno, "message": str(e), "type": "JSONDecodeError"}
                )
            ...
            for imp in imports:
                if code.count(imp) == 1:  # מופיע רק ב-import
                    result["suggestions"].append(
                        {"message": f"ייבוא לא בשימוש: {imp}", "type": "UnusedImport"}
                    )

        logger.info(
            f"נבדק תחביר עבור {programming_language}: {'תקין' if result['is_valid'] else 'לא תקין'}"
        )
        return result
    ...
    def highlight_code_batch(
        self, codes_data: List[Dict[str, str]], output_format: str = "html"
    ) -> Dict[str, str]:
