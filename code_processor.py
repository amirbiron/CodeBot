...
class CodeProcessor:
    ...
    def detect_language(self, code: str, filename: str = None) -> str:
        ...
        if language_scores:
            ...
            logger.info(
                f"זוהתה שפה לפי דפוסים: {detected} (ניקוד: {language_scores[detected]})"
            )
            return detected
    ...
    def create_code_image(
        ...
        try:
            ...
            img = Image.new(
                "RGB", (width, max(400, len(code.split("\n")) * 20)), "white"
            )
    ...
    def get_code_stats(self, code: str) -> Dict[str, Any]:
        ...
        stats["classes"] = len(
            re.findall(r"\bclass\s+\w+\s*[:\{]", code, re.IGNORECASE)
        )
    ...
    def extract_functions(
        self, code: str, programming_language: str
    ) -> List[Dict[str, str]]:
