...
| `codekeeper_search_notes` | חיפוש פתקים בשלושת המקומות שפתק יכול לשבת בהם — קובץ, לוח, או קובץ בריפו משוקף. לפי שם כברירת מחדל; `search_content=true` מרחיב גם לגוף הפתק, וזו הדרך למצוא פתק **בלי כותרת**. הפגיעה אומרת איפה הפתק יושב ולעולם אינה נושאת תוכן |
