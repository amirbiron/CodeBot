...
- **כן** נשלחים שני שדות של טקסט חופשי, ורק כשהערך מחרוזת: `$mcp_intent` (המשפט של הסוכן) ו-`$mcp_error_message` (הודעת החריגה, על כל סוג שגיאה). ההודעה עלולה לשאת קטע מהקלט שנדחה — האזהרה המלאה והמדידה שמאחוריה: `docs/mcp-server.rst`.
