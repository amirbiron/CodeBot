...
הזה היו אלה 16 קריאות. הלקוח שולח קריאות MCP לאותו שרת אחת אחרי השנייה גם
כשהסוכן מוציא אותן יחד, ולכן כל קריאה עולה זמן תעבורה משלה שאינו זמן
השרת. המספרים, ומאיפה הם, ב-``docs/mcp-server.rst`` (``codekeeper_read_batch``).
...
שהקבוצה הבאה נקראת. אין מטמון ואין פינוי: ההחלטה נגזרה ממדידה של הסבב
האמיתי — מתוך 14 פריטים רק שני זוגות חלקו קובץ — ולכן מטמון עם תקרה ופינוי
היה מנגנון שאין לו עבודה.
...
#:
#: הסבב שהוליד את הכלי קרא 15 מסמכים (PostHog, סשן ``ses_01a0cd48``,
...
#:
#: **החשבון, והנחה אחת שהוא נשען עליה.** Claude Code מחזיק לכל בקשה לשרת
#: HTTP טיימר של 60 שניות (``code.claude.com/docs/en/mcp.md``, נקרא ב-
#: 2026-09-23). הדדליין נבדק **בין פריטים בלבד** — פריט שהתחיל רץ עד סופו —
#: ולכן החריגה במקרה הגרוע היא הפריט היקר ביותר. היקר ביותר שנמדד הוא Markdown
#: עוין של 500KB: 2.3 שניות מעבד, שהן 46 שניות קיר כשעשרה חוטים חולקים חצי
#: מעבד. 10 + 46 = 56, מתחת ל-60. דדליין של 15 כבר היה 61.
