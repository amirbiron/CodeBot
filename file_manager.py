...
class VersionManager:
    ...
    def create_version(
        ...
        try:
            ...
            changes_summary = ""
            if latest:
                changes_summary = self._generate_changes_summary(latest["code"], content)
    ...
    def _generate_changes_summary(self, old_content: str, new_content: str) -> str:
        ...
        differ = difflib.unified_diff(old_lines, new_lines, fromfile="version_old", tofile="version_new", lineterm="")
    ...
    def compare_versions(self, user_id: int, file_name: str, version1: int, version2: int) -> Optional[FileDiff]:
        """השוואת שתי גרסאות"""
        ...
        try:
            ...
            diff = difflib.unified_diff(
                lines1, lines2, fromfile=f"version_{version1}", tofile=f"version_{version2}", lineterm=""
            )
...
class BackupManager:
    ...
    async def create_backup(
        ...
        try:
                ...
                for file_data in files:
                    ...
                    if include_versions:
                        ...
                        for version in versions[1:]:  # דלג על הגרסה הנוכחית
                            ...
                            version_filename = f"versions/{file_name}/v{version['version']}.txt"
            ...
            logger.info(f"נוצר גיבוי: {backup_id} ({file_count} קבצים, {total_size} bytes)")
            return backup_info
    ...
    def restore_from_backup(self, user_id: int, backup_path: str, overwrite: bool = False) -> Dict[str, Any]:
        """שחזור מגיבוי"""

        try:
            results = {"restored_files": 0, "skipped_files": 0, "errors": [], "files": []}
            ...
            with zipfile.ZipFile(backup_path, "r") as zip_file:
                ...
                # שחזור קבצים נוכחיים
                current_files = [name for name in zip_file.namelist() if name.startswith("current/")]
...
class FileExporter:
    ...
    async def export_files(
        self, user_id: int, file_names: List[str], export_format: str, options: Dict[str, Any] = None
    ) -> Optional[bytes]:
    ...
    async def _export_html(self, files_data: List[Dict], options: Dict) -> bytes:
        ...
        for file_data in files_data:
            ...
            if options.get("syntax_highlighting", True):
                highlighted = code_processor.highlight_code(code_content, file_data["programming_language"], "html")
            ...
            if file_data.get("tags"):
                tags_html = (
                    "<div class='tags'>"
                    + "".join([f"<span class='tag'>{tag}</span>" for tag in file_data["tags"]])
                    + "</div>"
                )
        ...
        html_content = html_template.format(
            export_date=datetime.now().strftime("%d/%m/%Y %H:%M"), file_count=len(files_data), files_html=files_html
        )
