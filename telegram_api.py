...
#:
#: **התחילית ``(?:[A-Za-z0-9]+[_.\-])*`` היא מה שמונע רשימה מתארכת.**
#: בלעדיה ``auth_token``, ``oauth_token``, ``id_token``, ``private_key``,
#: ``x-api-key`` ו-``session_token`` היו כולם דולפים — שמות פרמטרים נפוצים
#: לגמרי ב-OAuth ובקולבקים. פתרון של "נוסיף גם אותם לרשימה" היה משחזר בדיוק
#: את התקלה שהכלל הזה בא לחסל: רשימה שחורה שמתעדכנת רק אחרי דליפה.
#:
#: התחילית חייבת להסתיים במפריד (``_``, ``-``, ``.``), ולכן ``?monkey=``
#: **אינו** נתפס — המילה הרגישה חייבת להיות מקטע שלם בשם הפרמטר, לא סיומת
#: מקרית. יש טסט שנועל את שני הצדדים.
