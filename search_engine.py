...
class SearchIndex:
    ...
    def rebuild_index(self, user_id: int):
        ...
        logger.info(
            f"אינדקס נבנה: {len(self.word_index)} מילים, {len(self.function_index)} פונקציות"
        )
...
class AdvancedSearchEngine:
    ...
    def _text_search(
        self, query: str, index: SearchIndex, user_id: int
    ) -> List[SearchResult]:
        """חיפוש טקסט רגיל"""
    ...
    def _fuzzy_search(
        ...
        for file_data in files:
            ...
            name_ratio = fuzz.partial_ratio(
                query.lower(), file_data["file_name"].lower()
            )
    ...
    def _apply_filters(
        self, results: List[SearchResult], filters: SearchFilter
    ) -> List[SearchResult]:
        ...
        for result in results:
            ...
            if (
                filters.languages
                and result.programming_language not in filters.languages
            ):
                continue
            ...
            if filters.has_functions is not None:
                functions = code_processor.extract_functions(
                    result.content, result.programming_language
                )
    ...
    def _create_search_result(
        self, file_data: Dict, query: str, score: float
    ) -> SearchResult:
    ...
    def suggest_completions(
        self, user_id: int, partial_query: str, limit: int = 10
    ) -> List[str]:
        """הצעות השלמה לחיפוש"""
    ...
    def _get_most_common_words(
        self, index: SearchIndex, limit: int
    ) -> List[Tuple[str, int]]:
    ...
    def _get_most_common_languages(self, index: SearchIndex) -> List[Tuple[str, int]]:
        ...
        lang_counts = [
            (lang, len(files)) for lang, files in index.language_index.items()
        ]
