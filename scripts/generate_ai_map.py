...
def build_map() -> str:
    ...
    out.append("")
    # האזהרה נוסעת עם המפה ולא רק עם הפריימר: את הקובץ הזה קורא גם מי
    # שלא קיבל את הפריימר — סוכן בכלי אחר, או grep מקרי. הכלל שהוא הכי
    # קריטי לא ייקרא אם הוא גר רק במקום אחד.
    out.append("## סמכות התיעוד")
    out.append("")
    out.append(
        "התיעוד הוא **התמצאות, לא סמכות**. כל טענה עובדתית בו — התנהגות, "
        "נתיב, פרמטר, \"נתמך\" — קודם תאמת מול הקוד לפני שתסתמך עליה. "
        "התיעוד אינו תנ\"ך; ייתכן שאינו מעודכן. אמת מול הקוד."
    )
    out.append("")
    out.append(
        "שני חריגים: בלוקי `literalinclude` ועמודי autodoc, שנמשכים "
        "מהמקור בזמן הבנייה. סתירה בין פרוזה לקוד ← הקוד צודק, והסתירה "
        "ממצא לדיווח."
    )
    out.append("")
