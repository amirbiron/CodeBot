...
- **שבאץ' שחוצה את התקציב נעצר, והזיכרון אינו גדל איתו.** תשובה שמחכה לתורה
  נשמרת רק כל עוד יש לה סיכוי להיכנס (``read_batch._lower_bound``).

מה נמדד, ולמה דווקא כך:

- **בתים** — ``read_batch._wire``: בדיוק מה שה-SDK שולח, ובדיוק מה שהכלי
  עצמו מודד מול ``OUTPUT_BYTE_BUDGET``. לא נוסחה מקבילה.
- **זמן קיר** — ``time.perf_counter`` סביב הקריאה, בתהליך שאינו מריץ
  ``tracemalloc`` (שמאט כל הקצאה).
