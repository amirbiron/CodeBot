...
**למה הוא קיים בנוסף לטסט.** ``tests/test_md_parser_oracle.py`` רץ על
תת-קבוצה מייצגת שנבחרה **ידנית**, לפי הצורות שנמדדו ביום שהיא נבחרה —
ולכן צורה חדשה שתיכנס לריפו המקורי מחר לא תיתפס שם. הסקריפט הזה הוא מה
שסוגר את הפער: הוא מריץ את אותה השוואה בדיוק על הקורפוס החי.
