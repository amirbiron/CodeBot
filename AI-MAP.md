...
- `docs/dev/sticky_notes_extending.rst` — **עקרונות להוספת פיצ'ר לסטיקי-נוטס**: המלכודות החוזרות של הפתקים הדביקים: יעד יחיד ב-build_note_target, מכסה fail-closed, כוונה מול תצוגה בגודל ובמיקום, flush לפני פעולה הרסנית, אינדקס ממוספר, נורמליזציה בשני הקצוות, יתומים בקריאה, ועדכון עמוד המשתמש.
...
- `docs/user/sticky_notes.rst` — **פתקים דביקים (Sticky Notes)**: הצמדת הערות קצרות על תצוגת קובץ (Markdown), על לוח פתקים, או על קובץ בדפדפן הריפו הממורר: הוספה וניהול, עיגון יציב לעומת מיקום קבוע, התאמה לגבולות המסך במעבר בין מכשירים, מארקדאון, תזכורות, ויתומים.
