...
- `docs/dev/sticky_notes_extending.rst` — **עקרונות להוספת פיצ'ר לסטיקי-נוטס**: המלכודות החוזרות של הפתקים הדביקים במקום אחד — יעד יחיד ב-build_note_target, מכסה fail-closed והפטור-לאדמין שהורג מכסות, אינדקס ממוספר שנבנה לפני שמפילים, נורמליזציה בשני הקצוות, יתומים בקריאה, innerHTML אף פעם לא על טקסט לא-מהימן (תבנית סטטית — כן), ו-flush…
