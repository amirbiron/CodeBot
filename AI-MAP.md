...
- `docs/mcp-server.rst` — **שרת ה-MCP — חיבור Claude ל-CodeKeeper**: שרת ה-MCP שחושף את CodeKeeper ל-Claude: הכלים, האימות וההרשאות, פריימר הסוכן, עריכה מהדפדפן, ההתראה כשסוכן שומר קובץ, מדידת השימוש ושער הפרטיות שלה, וההפעלה צעד אחר צעד מול Claude.ai ומול Claude Code.
...
- `docs/user/sticky_notes.rst` — **פתקים דביקים (Sticky Notes)**: הצמדת הערות קצרות על תצוגת קובץ (Markdown), על לוח פתקים, או על קובץ בדפדפן הריפו הממורר: הוספה וניהול, עיגון יציב לעומת מיקום קבוע, בחירת צבע, מארקדאון, תזכורות, ויתומים.
