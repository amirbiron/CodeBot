...
class CodeErrorLogger:
    ...
    def log_code_processing_error(
        self, user_id: int, error_type: str, error_message: str, context: Dict[str, Any] = None
    ):
    ...
    def log_validation_failure(self, user_id: int, code_length: int, error_reason: str):
        ...
        self.log_code_processing_error(user_id, "validation_failure", error_reason, {"code_length": code_length})
...
class TimeUtils:
    ...
    def parse_date_string(date_str: str) -> Optional[datetime]:
        ...
        formats = ["%Y-%m-%d", "%d/%m/%Y", "%d.%m.%Y", "%Y-%m-%d %H:%M:%S", "%d/%m/%Y %H:%M", "%Y-%m-%dT%H:%M:%S"]
...
class SecurityUtils:
    ...
    @staticmethod
    def validate_user_input(text: str, max_length: int = 10000, forbidden_patterns: List[str] = None) -> bool:
        """בדיקת קלט משתמש"""
...
class FileUtils:
    ...
    @staticmethod
    async def create_temp_file(content: Union[str, bytes], suffix: str = "") -> str:
        """יצירת קובץ זמני"""
