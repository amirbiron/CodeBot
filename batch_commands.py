...
async def job_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        active_jobs = [
            job for job in batch_processor.active_jobs.values() if job.user_id == user_id
        ]
    ...
    if not job:
        await update.message.reply_text(
            f"❌ עבודת batch '{html_escape(job_id)}' לא נמצאה", parse_mode=ParseMode.HTML
        )
        return
    ...
    if job.status == "completed":
        ...
        keyboard.append([InlineKeyboardButton("🔄 רענן", callback_data=f"job_status:{job_id}")])
...
async def large_file_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not file_data:
        ...
        if suggestions:
            ...
            await update.message.reply_text(
                f"❌ קובץ '{html_escape(file_name)}' לא נמצא", parse_mode=ParseMode.HTML
            )
        return
...
async def handle_batch_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith("job_status:"):
            ...
            for file_name, result in job.results.items():
                adv = (
                    result.get("result", {}).get("advanced_checks")
            ...
            await query.edit_message_text(results_text, parse_mode=ParseMode.HTML)
...
def setup_batch_handlers(application):
    ...
    application.add_handler(
        CallbackQueryHandler(
            handle_batch_callbacks, pattern="^(job_status:|job_results:|job_cancel:|chunk:)"
        )
    )
