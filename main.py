...
def get_lock_collection():
    ...
    try:
        ...
        logger.critical(
            "'db.client' object not found or does not have 'get_default_database'. The structure of DatabaseManager might be different."
        )
        sys.exit(1)
...
class CodeKeeperBot:
    ...
    def setup_handlers(self):
        ...
        upload_conv_handler = ConversationHandler(
            entry_points=[CallbackQueryHandler(github_handler.handle_menu_callback, pattern="^upload_file$")],
            states={
                FILE_UPLOAD: [MessageHandler(filters.Document.ALL, github_handler.handle_file_upload)],
                REPO_SELECT: [MessageHandler(filters.TEXT & ~filters.COMMAND, github_handler.handle_text_input)],
                FOLDER_SELECT: [MessageHandler(filters.TEXT & ~filters.COMMAND, github_handler.handle_text_input)],
            },
        ...
        self.application.add_handler(
            MessageHandler(filters.TEXT & ~filters.COMMAND, handle_github_text), group=-1  # עדיפות גבוהה מאוד
        )
        ...
        # הוסף את ה-handler
        self.application.add_handler(
            MessageHandler(filters.Regex("^(ghp_|github_pat_)"), handle_github_token), group=0  # עדיפות גבוהה
        )
...
"""
    ...
    async def save_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        context.user_data["saving_file"] = {"file_name": file_name, "tags": tags, "user_id": user_id}
    ...
    async def list_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not files:
            await update.message.reply_text("📂 עדיין לא שמרת קטעי קוד.\n" "השתמש ב/save כדי להתחיל!")
            return
    ...
    async def search_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not results:
            await update.message.reply_text(
                f"🔍 לא נמצאו תוצאות עבור: <code>{' '.join(context.args)}</code>", parse_mode=ParseMode.HTML
            )
            return
    ...
    async def check_commands(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        personal_cmds = await context.bot.get_my_commands(scope=BotCommandScopeChat(chat_id=6865105071))
    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id in ADMIN_IDS:
            ...
            if not stats or stats.get("total_files", 0) == 0:
                await update.message.reply_text("📊 עדיין אין לך קטעי קוד שמורים.\n" "התחל עם /save!")
                return
...
💡 **טיפ:** השתמש בתגיות לארגון טוב יותר!
    ...
    async def handle_document(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if document.file_size > 10 * 1024 * 1024:
                await update.message.reply_text("❌ הקובץ גדול מדי!\n" "📏 הגודל המקסימלי המותר הוא 10MB")
                return
            ...
            if len(content) > 4096:
                ...
                snippet = CodeSnippet(user_id=user_id, file_name=file_name, code=content, programming_language=language)
    ...
    async def _save_code_snippet(self, update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
        ...
        if len(code) > config.MAX_CODE_SIZE:
            await update.message.reply_text(f"❌ הקוד גדול מדי! מקסימום {config.MAX_CODE_SIZE} תווים.")
            return
    ...
    def _looks_like_code(self, text: str) -> bool:
        ...
        return any(indicator in text for indicator in code_indicators) or text.count("\n") > 3 or text.count("{") > 1
...
def setup_handlers(application: Application, db_manager):  # noqa: D401
    ...
    async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):  # noqa: D401
        ...
        await update.message.reply_text(
            "👋 שלום! הבוט מוכן לשימוש.\n\n" "🔧 לכל תקלה בבוט נא לשלוח הודעה ל-@moominAmir", reply_markup=reply_markup
        )
...
async def setup_bot_data(application: Application) -> None:  # noqa: D401
    ...
    try:
        ...
        await application.bot.set_my_commands(
            commands=[BotCommand("stats", "📊 סטטיסטיקות שימוש")], scope=BotCommandScopeChat(chat_id=AMIR_ID)
        )
