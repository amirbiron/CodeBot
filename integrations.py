...
class GitHubGistIntegration:
    ...
    def create_gist(
        self, file_name: str, code: str, language: str, description: str = "", public: bool = True
        ...
        try:
            ...
            files = {file_name: {"content": code}}

            # יצירת ה-Gist
            gist = self.user.create_gist(public=public, files=files, description=description)
    ...
    def update_gist(self, gist_id: str, file_name: str, new_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # עדכון הקובץ
            files = {file_name: {"content": new_code}}

            gist.edit(files=files)

            logger.info(f"עודכן Gist: {gist.html_url}")
            return {"id": gist.id, "url": gist.html_url, "updated_at": datetime.now().isoformat()}
...
class PastebinIntegration:
    ...
    async def create_paste(
        self, code: str, file_name: str, language: str = None, private: bool = True, expire: str = "1M"
    ) -> Optional[Dict[str, Any]]:
        """יצירת paste חדש"""
...
class CodeSharingService:
    ...
    async def share_code(
        self, service: str, file_name: str, code: str, language: str, description: str = "", **kwargs
    ) -> Optional[Dict[str, Any]]:
    ...
    def _create_internal_share(self, file_name: str, code: str, language: str, description: str) -> Dict[str, Any]:
...
class GitRepositoryIntegration:
    ...
    def create_repository_file(
        self, repo_name: str, file_path: str, content: str, commit_message: str = None
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # יצירת הקובץ
            result = repo.create_file(path=file_path, message=commit_message, content=content)
...
class WebhookIntegration:
    ...
    def register_webhook(self, user_id: int, webhook_url: str, events: List[str] = None) -> str:
        """רישום webhook"""
    ...
    async def trigger_webhook(self, user_id: int, event: str, data: Dict[str, Any]):
        ...
        payload = {"event": event, "timestamp": datetime.now().isoformat(), "user_id": user_id, "data": data}
