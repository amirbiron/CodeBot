...
class GitHubGistIntegration:
    ...
    def update_gist(
        self, gist_id: str, file_name: str, new_code: str
    ) -> Optional[Dict[str, Any]]:
        """עדכון Gist קיים"""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/api_post.php", data=data
                ) as response:
                    result = await response.text()

                    if response.status == 200 and result.startswith(
                        "https://pastebin.com/"
                    ):
    ...
    async def share_code(
        ...
        **kwargs,
        ...
        if service == "gist" and self.gist.is_available():
            return self.gist.create_gist(
                file_name, code, language, description, **kwargs
            )
    ...
    def create_repository_file(
        ...
        try:
            ...
            result = repo.create_file(
                path=file_path, message=commit_message, content=content
            )
...
class WebhookIntegration:
    ...
    def register_webhook(
        self, user_id: int, webhook_url: str, events: List[str] = None
    ) -> str:
        """רישום webhook"""
