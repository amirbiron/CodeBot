        ...
        try:
            ...
            if deleted_count > 0:
                ...
                await query.edit_message_text(
                    text="⚠️ לא נמצא קובץ למחיקה. ייתכן שכבר נמחק."
                )
...
def setup_advanced_handlers(application: Application, db):
    ...
    application.add_handler(
        CallbackQueryHandler(handlers._delete_file, pattern="^delete_")
    )
    application.add_handler(
        CallbackQueryHandler(handlers._download_file, pattern="^download_")
    )
    application.add_handler(
        CallbackQueryHandler(handlers._share_file, pattern="^share_")
    )
