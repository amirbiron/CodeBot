...
#
# **גם קבצים מקומפלים, ולא רק ספריות.** המיפוי הראשון ספר קבצים ומצא
# שמחוץ ל-``node_modules`` יש בודדים — אבל מה שקובע הוא שורות תואמות, לא
# קבצים: ``md_preview.bundle.js`` הוא קובץ אחד שמחזיק 8,261 מתוך 12,979
# ההתאמות של ``function`` בריפו הזה (64%), ‏11,638 מ-``return``, ‏7,754
# מ-``const`` — נמדד על המראה, ספטמבר 2026. ‏``*.min.js``/``*.map``/``*.min.css``
# תורמים כאן מעט (שורה אחת לקובץ), אבל הם אותה מחלקה בדיוק: פלט בנייה
# שאיש לא כתב ולא יערוך. רשימה אחת, מנגנון אחד, ו-``include_vendored``
# מחזיר את כולם יחד.
VENDORED_PATH_GLOBS: tuple = (
    "node_modules/*",
    "*.bundle.js",
    "*.min.js",
    "*.min.css",
...
#
# **למה דווקא כאן, ומה בכלל עוצר בפועל.** נמדד על מראה בערה של CodeBot
# (git 2.43, ספטמבר 2026), עם החרגת הקוד החיצוני פעילה: **עלות הספירה
# כמעט אינה תלויה במספר ההתאמות.** ‏``git grep -c`` סופר בעצמו ומדפיס
# שורה אחת לכל קובץ תואם ולא שורה לכל התאמה, ולכן ספירה שלמה של שאילתת
# מילה (עשרות אלפי התאמות) ושל האות ``e`` (מעל חצי מיליון) נמדדו שתיהן
# ב-0.14–0.16 שניות.
#
# מכאן שתי מסקנות שקובעות את הערך:
#
# 1. **התקרה, ולא ה-timeout, היא מה שעוצר.** ספירה מלאה של השאילתה הרחבה
#    ביותר עלתה 0.145 שניות — ‏1.5% מתקציב עשר השניות. כדי שה-timeout
#    יקדים את התקרה צריך ריפו גדול בסדר גודל, או דיסק איטי בהרבה.
# 2. **התקרה נדלקת רק על שאילתות תו בודד.** כל שאילתה שנראית כמו מונח
#    בקוד מסתיימת הרבה מתחתיה ומקבלת ``total`` מדויק. היא קיימת בשביל
#    הקצה הפתולוגי, לא בשביל השימוש היומיומי.
#
# והבחירה במספר עצמו: מעל 100,000 התאמות המספר המדויק ממילא אינו משנה שום
# החלטה של סוכן, והעצירה שם חוסכת כשני שלישים מזמן הספירה (40ms במקום
# 150ms) על אותן שאילתות רחבות.
...
def get_mirror_service() -> GitMirrorService:
    ...
    **בלי נעילה, וזו החלטה ולא השמטה.** מאז שגופי כלי ה-MCP ירדו מלולאת
    האירועים לחוטי עובד (#3379) שני קוראים יכולים להגיע לכאן יחד, ולכן שווה
    לומר מה זה עולה: כלום. הגלובל שנבדק הוא הגלובל שמוחזר, כלומר אין חלון
    שבו האחד מוצב והשני ריק; וההצבה קורית רק אחרי שהבנאי חזר, כך שכשל
    (למשל ``PermissionError`` מ-``_ensure_base_path``) משאיר אותו ריק
    והקריאה הבאה מנסה באמת ולא חוזרת מיד עם ``None``.

    בנייה כפולה בטוחה: הבנאי מציב נתיבים ותבניות, ותופעת הלוואי היחידה שלו
    היא ``mkdir(parents=True, exist_ok=True)``. המופע שמפסיד את המרוץ נזרק
    בזמן שהקורא שלו עדיין מחזיק בו — וגם זה בסדר, כי אין מצב פר-מופע מעבר
    לאותם נתיבים, ושני המופעים עונים אותו דבר.
    """
