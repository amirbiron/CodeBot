...
מעביר ל-``rst_parser.parse_document`` את תקרת הסקשנים של הסורק (מאז הסקירה
של #3429) ותופס ``TooManySections`` בלבד — זו החריגה היחידה ש-``rst_parser``
מרים; ``InconsistentLineEndings`` אינה נתפסת שם. מי שיחליף שם פארסר מכניס
מסלול חריגה שני לאתר הקריאה, והמרתו לתשובת ``error`` של MCP היא עבודה של
אותו שינוי — לא של המודול הזה, שאינו יודע דבר על MCP.
