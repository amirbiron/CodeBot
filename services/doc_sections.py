...
ולקרוא לאותם שמות. **בדיוק אותם שמות — למעט אחד:** ``md_parser`` מייצא
גם ``InconsistentLineEndings``, ש-``rst_parser`` אינו מרים ואינו מגדיר.
חריגות הסירוב נתפסות לכן דרך המודול הזה, לא דרך הפארסר — ראו את
ה-docstring של :class:`InconsistentLineEndings`.

**ואותה ברירת מחדל ל-``max_sections`` בשניהם — :data:`MAX_SECTIONS`, שמוגדר
כאן.** עד #3420 היא הייתה ``None`` ב-``rst_parser`` — מה ששמר על
``docs_get_section`` זהה בית-בית כשהפרמטר נוסף לפארסר שכבר רץ בייצור — ו-
``MAX_SECTIONS`` ב-``md_parser``, כלומר קורא ששכח להעביר תקרה קיבל הגנה
מהפארסר האחד ולא מהשני. היישור נעשה אחרי מדידה: הקובץ העשיר ביותר ב-``docs/``
נושא 50 סקשנים מול תקרה של 50,000, ופלט ``docs_get_section`` על כל 208 קובצי
ה-RST זהה לפני ואחרי. ההיגיון של ברירת המחדל כתוב ליד הקבוע.

**ומי הבעלים של שני ערוצי הסירוב: המתקשר.** ``mcp_server/docs_handlers.py``
בוחר פארסר לפי סיומת, משאיר את שניהם על ברירת המחדל שלהם (בין #3429 ל-#3420
הוא העביר ל-``rst_parser`` את תקרת הסורק במפורש, כי ברירת המחדל שם הייתה
``None``), ו**תופס את שתי החריגות** — ``TooManySections`` ו-
``InconsistentLineEndings`` — מהמודול הזה ולא מהפארסר, כך שהתפיסה אינה
מותנית במי שפרסר. שתיהן הופכות לתשובת ``error`` של MCP עם מספר השורה שהן
נושאות. ההמרה הזאת היא עבודה של המתקשר — לא של המודול הזה, שאינו יודע דבר
על MCP.

**ובדיקת הכניסה זהה בשניהם — :func:`require_str`, שמוגדרת כאן.** עד #3421
``rst_parser.parse_document(None)`` החזיר ``Document`` ריק **בשקט** — מפה
ריקה שמתחזה למפה של קובץ בלי כותרות — ו-``rst_parser.parse_document(17)``
זרק ``AttributeError`` גולמי, בעוד ``md_parser`` זרק ``TypeError`` שאומר
מה התקבל. עכשיו שניהם עוברים באותה בדיקה ומרימים את אותה חריגה עם אותן
מילים; אף קורא לא נשען על הצורה הישנה (נבדק: המטפל, הסורק והסקריפטים
מעבירים תמיד מחרוזת).
...
#: מקסימום יחידות עבודה לפרסור אחד, ומעליו הפרסור נעצר ב-:class:`TooManySections`.
#: זו **ברירת המחדל** של ``max_sections`` בשני הפארסרים (מאז #3420), ו-``None``
#: מכבה אותה במפורש.
#:
#: **מה נספר תלוי בפארסר, וזה ההבדל שצריך להכיר.** ב-``rst_parser`` נספרים
#: סקשנים; ב-``md_parser`` נספרת כל **כותרת** שהפרסור נתקל בה, כולל כותרת
#: בתוך ציטוט או בתוך פריט רשימה שאינה נכנסת למפה בכלל. הקבוע מגביל
#: **עבודה** ולא **תוצאה**: מה שמייקר הוא הכותרת עצמה, ולא השאלה אם היא
#: נספרת כסעיף בסוף. נמדד: 500KB של ``> ## h`` הם ~64,000 כותרות שאף
#: אחת מהן אינה סעיף, וספירה לפי ``level == 0`` הייתה רואה בהן אפס ולא
#: עוצרת לעולם. הטענה "לא יותר מ-``MAX_SECTIONS`` סעיפים" נשארת נכונה — היא
#: פשוט לא כל מה שהקבוע אוכף. שם מדויק יותר לקבוע הוא שינוי בפני עצמו
#: והוא אישו נפרד.
#:
#: **ולמה ברירת מחדל ולא ``None``:** קורא ששכח להעביר תקרה מקבל הגנה ולא
#: את היעדרה — אותו כלל fail-closed שכבר מנומק ב-``_declares_write``
#: וב-``_ceiling.py``. המחיר שלה על קלט אמיתי הוא אפס: הקובץ העשיר ביותר
#: ב-``docs/`` נושא 50 סקשנים, אחד לאלף מהתקרה (נמדד על כל 208 קובצי
#: ה-RST, 2026-09-20), ופלט ``docs_get_section`` על כולם זהה בית-בית
#: לפני היישור ואחריו.
#:
...
#: אותה מפה, וסורק האאוטליין מעביר את זה במקום את זה. השוויון
...
def require_str(text: object) -> str:
    ...
    הטקסט מגיע מחוץ לתהליך (קובץ מהמראה, גוף בקשה), ו-``.replace`` על ערך
    שאינו מחרוזת היה מפיל ``AttributeError`` ממקום שלא מסביר כלום — או, גרוע
    מזה, ``None`` שהופך ל-``""`` ומחזיר מפה ריקה שמתחזה למפה של קובץ בלי
    כותרות (זו הצורה שהייתה ב-``rst_parser`` עד #3421). החריגה אומרת מה
    התקבל, כי זה מה שמבדיל בין "הקלט פסול" ל"הקוד הקורא שבור".

    **פונקציה אחת לשני הפארסרים ולא שתי שורות זהות בכל אחד**, כדי שהמילים
    יישארו אותן מילים: ``TypeError`` היא חוזה שהמטפל מצהיר עליו ("חוזה
    נשבר" ולא "הקלט נדחה", ולכן אינה נתפסת ב-``docs_get_section``), ושני
    עותקים של אותה בדיקה הם מה שנסחף בתיקון הבא.
    """
...
class TooManySections(Exception):
    ...
    **ומה בדיוק נספר תלוי בפארסר, וזה מכוון.** ב-``rst_parser`` נספרים
    ...
    כמו כל אחרת — ההנמקה המלאה ליד :data:`MAX_SECTIONS` למעלה.
    המחלקה אחת כי מה שהקורא צריך לדעת זהה: הקלט גדול מדי, ואין מפה.

    **חריגה, ולא ערך החזרה ולא ``Document`` חלקי עם דגל "נקטע".** המתקשר
    היחיד שמעביר תקרה במפורש הוא סורק האאוטליין (השאר על ברירת המחדל),
    ושם הכלל כבר נקבע ומנומק
...
#:
#: **וזה גם מה שסוכן מקבל בפועל** — ``mcp_server/docs_handlers.py`` מבקש
#: את המספר הזה במפורש, כי התשובה "ביקשת מזהה שאינו קיים, אלה שכן" היא
#: **מלאי ולא דירוג**: אין בה סדר עדיפות, ולכן חיתוך שלה מסתיר פריטים בלי
#: שום קריטריון — ולסוכן אין פרמטר לבקש את השאר. לכן **זה המספר שנכנס
#: לתיאור הפרמטר**.
MAX_IDENTIFIER_SUGGESTIONS = 50

#: ברירת המחדל של :func:`suggest` כשהקורא לא ביקש מספר.
#:
#: **היא שייכת למסלול ה-``difflib`` ולא למסלול המזהים**, וההבדל אינו
#: קוסמטי: ``difflib`` מחזיר **דירוג** — חמש הכותרות הקרובות ביותר הן
#: באמת התשובה, והשישית פחות טובה. מלאי מזהים אינו מדורג, ולכן חמישה
