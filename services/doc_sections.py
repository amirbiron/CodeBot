...
בוחר פארסר לפי סיומת, מעביר ל-``rst_parser.parse_document`` את תקרת הסקשנים
של הסורק (מאז #3429) ומשאיר את ``md_parser`` על ברירת המחדל שלו, ו**תופס את
שתי החריגות** — ``TooManySections`` ו-``InconsistentLineEndings`` — מהמודול
הזה ולא מהפארסר, כך שהתפיסה אינה מותנית במי שפרסר. שתיהן הופכות לתשובת
``error`` של MCP עם מספר השורה שהן נושאות. ההמרה הזאת היא עבודה של המתקשר
— לא של המודול הזה, שאינו יודע דבר על MCP.
