...
**למה מודול נפרד, ולא "פשוט להשאיר ב-``rst_parser``".** מתוכנן צרכן שני
לאותו חוזה — פארסר Markdown (``services/md_parser.py``, ראו למטה מה
שאינו קיים עדיין). שני פארסרים שבונים ``Section`` משלהם ומחשבים
...
**ומה שעדיין לא קיים, כדי שהקורא לא ישלים מהדמיון:** נכון לכתיבת
השורות האלה יש **פארסר אחד בלבד** שבונה לתוך המודול הזה —
``services/rst_parser.py``. ``services/md_parser.py`` **אינו קיים**, והוא
ה-PR הבא בתוכנית. כלומר המודול הזה נכתב עבור צרכן שני מתוכנן, לא עבור
צרכן שני שכבר עובד.
