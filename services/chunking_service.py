...
def create_embedding_text(
    ...
    **התקרה האפקטיבית של הקלט אינה ``CHUNK_MAX_BYTES``.** המחרוזת המקסימלית
    כאן היא ``EMBEDDING_METADATA_MAX_BYTES + len("\n\nCode:\n") +
    CHUNK_MAX_BYTES`` — עם ברירות המחדל: 500 + 8 + 2,000 = **2,508 בייט**.
    זה המספר שצריך להשוות לחלון הקלט של המודל, ולא 2,000. מי שמשנה את
    ``CHUNK_MAX_BYTES`` צריך לחשב מחדש לפי הנוסחה ולא לפי המספר.

    **החלון נמדד מול ה-API החי** (``scripts/probe_embedding_limits.py``),
    בשיטה של "הוסף זנב וראה אם הווקטור זז": כשהוא לא זז, הקלט כבר מילא את
    החלון והזנב נחתך. הנקודה שבה החלון מתמלא:

    ==================================  ==================
    תוכן                                החלון מתמלא סביב
    ==================================  ==================
    פרוזה באנגלית                       ~12,600 בייט
    עברית                               ~7,500 בייט
    base64                              ~2,950 בייט
    סימנים צפופים (נתיב SVG, JS ממוזער) ~2,200 בייט
    ==================================  ==================

    כל סוגי התוכן האמיתיים בריפו — פייתון, מארקדאון עברי, RST, YAML, CSS,
    JS — נבדקו ב-2,508 בייט ו**נקראו במלואם**. התוכן היחיד שגולש הוא סימנים
    צפופים, וזה בדיוק מה ש-``is_low_information_chunk`` מדלג עליו. הפער
    שנשאר: תוכן צפוף-סימנים שיחסו מתחת ל-0.8 (למשל JS ממוזער סינתטי ביחס
    0.797) — מקרה קצה שאינו בקורפוס, ומתועד כאן כדי שיהיה ידוע.
    """
    parts: List[str] = []
