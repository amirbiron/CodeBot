...
async def upload_to_github_fixed(update, context, status_message):
    ...
    if not github_token:
        await status_message.edit_text("❌ אין חיבור לגיטהאב.\n" "השתמש ב-/github להתחברות או הגדר טוקן.")
        return
    ...
    try:
        ...
        try:
            ...
            result = repo.create_file(
                path=file_path, message=f"Upload {file_name} via Telegram bot", content=file_bytes
            )
            action = "הועלה"
        ...
        await status_message.edit_text(
            success_message, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard)
        )
        ...
        await status_message.edit_text(f"❌ שגיאה כללית:\n`{str(e)}`", parse_mode="Markdown")
...
async def stats_command_secured(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != AMIR_ID:
        await update.message.reply_text("⛔ הפקודה הזו זמינה רק למנהל המערכת.")
