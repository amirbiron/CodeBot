...
async def show_all_files(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not files:
            ...
            files_count_text = (
                f"({len(files)} קבצים)" if len(files) <= 10 else f"({len(files)} קבצים - מציג 10 הטובים ביותר)"
            )
...
async def show_all_files_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if regular_files:
            keyboard.append(
                [InlineKeyboardButton(f"📁 קבצים רגילים ({len(regular_files)})", callback_data="show_regular_files")]
            )

        if large_files:
            keyboard.append(
                [InlineKeyboardButton(f"📚 קבצים גדולים ({large_count})", callback_data="show_large_files")]
            )
        ...
        if not regular_files and not large_files:
            ...
            text = f'📚 **הקבצים שלך** (סה"כ: {total_files})\n\n' "🎯 בחר קטגוריה:"
...
async def handle_duplicate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith("replace_"):
        ...
        await query.edit_message_text("✏️ *שנה שם קובץ*\n\n" "📝 שלח שם קובץ חדש:", parse_mode="Markdown")
        return GET_FILENAME
    elif data == "cancel_save":
        context.user_data.clear()
        await query.edit_message_text("🚫 השמירה בוטלה!")
        await query.message.reply_text(
            "🏠 חוזרים לתפריט הראשי:", reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
        )
        return ConversationHandler.END
...
async def receive_new_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if success:
            ...
            await update.message.reply_text(
                "❌ שגיאה בעדכון הקוד", reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
            )
...
async def receive_new_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if success:
            ...
            await update.message.reply_text(
                "❌ שגיאה בשינוי השם", reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
            )
...
async def handle_download_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await query.edit_message_text(
            f"✅ *הקובץ הורד בהצלחה!*\n\n" f"📄 **שם:** `{file_name}`", reply_markup=reply_markup, parse_mode="Markdown"
        )
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if data.startswith("file_") and not data.startswith("files"):
            ...
            await query.message.reply_text(
                "🎮 בחר פעולה מתקדמת:", reply_markup=ReplyKeyboardMarkup(MAIN_KEYBOARD, resize_keyboard=True)
            )
