    ...
    async def show_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "📄 אנא ציין שם קובץ:\n" "דוגמה: `/show script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        if not file_data:
            await update.message.reply_text(f"❌ קובץ `{file_name}` לא נמצא.", parse_mode=ParseMode.MARKDOWN)
            return

        # קבל את הקוד המקורי (הפונקציה highlight_code תחזיר אותו כפי שהוא)
        original_code = code_processor.highlight_code(file_data["code"], file_data["programming_language"], "html")
...
"""
    ...
    async def edit_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "✏️ אנא ציין שם קובץ לעריכה:\n" "דוגמה: `/edit script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        # שמירת מידע לעריכה
        context.user_data["editing_file"] = {"file_name": file_name, "user_id": user_id, "original_data": file_data}
    ...
    async def delete_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "🗑️ אנא ציין שם קובץ למחיקה:\n" "דוגמה: `/delete script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    async def versions_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "🔢 אנא ציין שם קובץ:\n" "דוגמה: `/versions script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        for version_data in versions[:5]:  # מקסימום 5 גרסאות בכפתורים
            keyboard.append(
                [
                    ...
                    InlineKeyboardButton(
                        f"🔄 שחזר", callback_data=f"restore_version_{file_name}_{version_data['version']}"
                    ),
                ]
            )
    ...
    async def analyze_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "📊 אנא ציין שם קובץ לניתוח:\n" "דוגמה: `/analyze script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
...
• ניקוד קריאות: {stats.get('readability_score', 'לא זמין')}
    ...
    async def validate_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "✅ אנא ציין שם קובץ לבדיקה:\n" "דוגמה: `/validate script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    async def share_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "🌐 אנא ציין שם קובץ לשיתוף:\n" "דוגמה: `/share script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    async def download_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "📥 אנא ציין שם קובץ להורדה:\n" "דוגמה: `/download script.py`", parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    async def recent_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        recent_files = [f for f in files if f["updated_at"] >= since_date]

        if not recent_files:
            await update.message.reply_text(f"📅 לא נמצאו קבצים שעודכנו ב-{days_back} הימים האחרונים.")
            return
        ...
        try:
            if data.startswith("confirm_delete_"):
                ...
                if db.delete_file(user_id, file_name):
                    await query.edit_message_text(f"✅ הקובץ `{file_name}` נמחק בהצלחה!", parse_mode=ParseMode.MARKDOWN)
                else:
    ...
    async def _send_highlighted_code(self, query, user_id: int, file_name: str):
        ...
        # יצירת קוד מודגש
        highlighted = code_processor.highlight_code(file_data["code"], file_data["programming_language"], "html")
        ...
        if len(file_data["code"]) > 500:
            ...
            await query.message.reply_document(
                document=InputFile(html_file, filename=f"{file_name}.html"), caption=f"🎨 קוד מודגש עבור `{file_name}`"
            )
        else:
            # שליחה כהודעה
    ...
    async def _share_to_gist(self, query, user_id: int, file_name: str):
        ...
        if not config.GITHUB_TOKEN:
            await query.edit_message_text("❌ שיתוף ב-Gist לא זמין - לא הוגדר טוקן GitHub.")
            return
