...
class BatchProcessor:
    ...
    def create_job(self, user_id: int, operation: str, files: List[str]) -> str:
        ...
        job = BatchJob(job_id=job_id, user_id=user_id, operation=operation, files=files)
    ...
    async def process_files_batch(
        self, job_id: str, operation_func: Callable, **kwargs
    ) -> BatchJob:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        job.results[file_name] = {"success": success_flag, "result": result}
    ...
    async def validate_files_batch(self, user_id: int, file_names: List[str]) -> str:
        ...
        def _run_local_cmd(args_list, cwd: str, timeout_sec: int = 20) -> Dict[str, Any]:
            try:
                completed = subprocess.run(
                    args_list, cwd=cwd, capture_output=True, text=True, timeout=timeout_sec
                )
        ...
        def _copy_lint_configs(temp_dir: str) -> None:
            ...
            configs = [
                ...
                (
                    os.path.join(project_root, ".eslintrc.json"),
                    os.path.join(temp_dir, ".eslintrc.json"),
                ),
                (
                    os.path.join(project_root, ".eslintrc.js"),
                ...
                (
                    os.path.join(project_root, "prettier.config.js"),
                    os.path.join(temp_dir, "prettier.config.js"),
                ),
                (
                    os.path.join(project_root, "tsconfig.json"),
                    os.path.join(temp_dir, "tsconfig.json"),
                ),
                # YAML/Docker/Semgrep
                (os.path.join(project_root, ".yamllint"), os.path.join(temp_dir, ".yamllint")),
                (
                    os.path.join(project_root, ".hadolint.yaml"),
                    os.path.join(temp_dir, ".hadolint.yaml"),
                ),
                (
                    os.path.join(project_root, ".semgrep.yml"),
                    os.path.join(temp_dir, ".semgrep.yml"),
                ),
                (
                    os.path.join(project_root, ".semgrep.yaml"),
                    os.path.join(temp_dir, ".semgrep.yaml"),
                ),
            ]
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(prefix="validate_") as temp_dir:
                    ...
                    if (language or "").lower() == "python" or temp_file.endswith(".py"):
                        ...
                        adv["pylint"] = _run_local_cmd(
                            ["pylint", "-sn", os.path.basename(temp_file)], temp_dir
                        )
                        adv["isort"] = _run_local_cmd(
                            ["isort", "--check-only", os.path.basename(temp_file)], temp_dir
                        )
                        adv["radon_cc"] = _run_local_cmd(
                            ["radon", "cc", "-s", "-a", os.path.basename(temp_file)], temp_dir
                        )
                        adv["radon_mi"] = _run_local_cmd(
                            ["radon", "mi", os.path.basename(temp_file)], temp_dir
                        )

                    # JavaScript / TypeScript
                    if temp_file.endswith(".js") or temp_file.endswith(".ts"):
                        adv["eslint"] = _run_local_cmd(
                            ["eslint", "-f", "stylish", os.path.basename(temp_file)], temp_dir
                        )
                        if temp_file.endswith(".ts"):
                            adv["tsc"] = _run_local_cmd(
                                ["tsc", "--noEmit", os.path.basename(temp_file)], temp_dir
                            )
                        adv["prettier"] = _run_local_cmd(
                            ["prettier", "--check", os.path.basename(temp_file)], temp_dir
                        )

                    # Shell scripts
                    if temp_file.endswith(".sh"):
                        adv["shellcheck"] = _run_local_cmd(
                            ["shellcheck", os.path.basename(temp_file)], temp_dir
                        )

                    # YAML
                    if temp_file.endswith(".yml") or temp_file.endswith(".yaml"):
                        adv["yamllint"] = _run_local_cmd(
                            ["yamllint", os.path.basename(temp_file)], temp_dir
                        )

                    # Dockerfile
                    if ext == "dockerfile" or os.path.basename(temp_file).lower() == "dockerfile":
                        adv["hadolint"] = _run_local_cmd(
                            ["hadolint", os.path.basename(temp_file)], temp_dir
                        )

                    # JSON
                    if temp_file.endswith(".json"):
                        adv["jq"] = _run_local_cmd(
                            ["jq", "-e", ".", os.path.basename(temp_file)], temp_dir
                        )

                    # Semgrep (generic SAST) - optional and quiet
                    adv["semgrep"] = _run_local_cmd(
                        ["semgrep", "--quiet", "--config", "auto", os.path.basename(temp_file)],
                        temp_dir,
                    )
    ...
    async def export_files_batch(
        self, user_id: int, file_names: List[str], export_format: str = "zip"
    ) -> str:
        ...
        # הרצה ברקע כדי לא לחסום את הלולאה הראשית
        asyncio.create_task(
            self.process_files_batch(job_id, export_single_file, format=export_format)
        )
        return job_id
