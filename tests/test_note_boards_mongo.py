...
מתי הן רצות: כש-``MONGODB_URL`` מוגדר והשרת נענה. ב-CI של ה-PR
(``.github/workflows/ci.yml``) הן **מדלגות**: הג'וב ``Unit Tests`` אמנם מרים
``mongo:8.0`` כשירות, אבל בלי ``ports:`` ובלי ``container:``, ולכן שם המארח
``mongodb`` אינו נפתר. אחרי מיזוג הן כן רצות — ב-``deploy.yml`` לשירות יש
``ports:`` וה-``MONGODB_URL`` מצביע ל-``localhost``. מקומית הן מדלגות, כדי
שהרצה רגילה תישאר מהירה. ראו ``docs/testing.rst``.
