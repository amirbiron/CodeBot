...
class TestGitHubIssueAction:
    ...
    @pytest.mark.asyncio
    async def test_execute_uses_params_title_when_title_template_missing(self, action, sample_alert):
        """UI חדש שומר title בתוך params.title; ה-Backend צריך לכבד זאת."""
        action_config = {
            "type": "create_github_issue",
            "labels": ["auto-generated", "bug"],
            "params": {"title": "🐛 {{service_name}}: {{error_message}}"},
        }
