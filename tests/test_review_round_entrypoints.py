...
על טסטי ה-grep שבקובץ
---------------------
שלושה טסטים כאן קוראים את קוד המקור ומחפשים מחרוזות:
``test_the_script_never_calls_create_index_directly``,
``test_the_target_comes_from_the_same_source_as_the_worker``, ו-
``test_main_has_no_leftover_inline_flag_parsing``. זה **בכוונה**, והם אינם
תחליף לטסטים ההתנהגותיים שלצידם — כל אחד מהם יושב ליד טסט שבודק את אותה
התנהגות באמת (הקריאה ל-``safe_create_index``, הנרמול בפועל, ``_env_flag``
על כל קבוצת הערכים).

מה שהם כן שומרים עליו הוא דבר שהתנהגות אינה יכולה לתפוס: **שהתיקון לא
יחזור לאחור בשקט בעותק שני**. הבאג המקורי לא היה "``safe_create_index``
לא עובד" אלא "מישהו קרא ל-``create_index`` הגולמי במקום"; הוספת אתר קריאה
חמישי עם הביטוי הישן של הדגל אינה שוברת שום התנהגות קיימת. טסט התנהגותי
עובר בשמחה בשני המקרים.

המחיר ידוע: rename או פירמוט יפילו אותם בלי רגרסיה אמיתית. זו עסקה מודעת,
והודעות הכשל מנוסחות כדי שיהיה ברור מיד שזה מה שקרה.
"""
