...
@pytest.mark.asyncio
async def test_maintenance_message_uses_app_bot_and_user_fallback(monkeypatch):
    monkeypatch.setenv('BOT_TOKEN', 'x')
    monkeypatch.setenv('MONGODB_URL', 'mongodb://localhost:27017/test')
    monkeypatch.setenv('DISABLE_DB', '1')
    monkeypatch.setenv('MAINTENANCE_MODE', 'true')
    ...
    context = types.SimpleNamespace(app=fake_app)

    await maint_msg_handler.callback(update, context)

    assert fake_bot.sent, 'expected send_message fallback via app.bot when no chat info available'
    assert fake_bot.sent[-1] == (777, mod.config.MAINTENANCE_MESSAGE)
