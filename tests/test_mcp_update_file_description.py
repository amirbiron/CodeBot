...
async def test_the_latest_version_is_chosen_by_an_explicit_sort_not_by_luck():
    ...
    **והעדכון הוא pipeline מאז שנוספה חותמת גיל התיאור.** החותמת היא
    ...
    ש-``find_one_and_update`` נבחר כדי לסגור. הצורה נטענת כאן ולא
    בהתנהגות מאותה סיבה שה-``sort`` נטען כאן: מימוש שיחליף את
    ה-``$version`` במספר שנקרא מראש ייראה זהה בכל בדיקה התנהגותית.
