...
def bot(monkeypatch):
    ...
    class _DB:
        def get_favorites(self, uid, limit=50):
            return [{'file_name': 'rep.py', 'programming_language': 'python'}]
...
@pytest.mark.asyncio
async def test_favorites_list_callback(bot):
    q = _Query(); q.data = "favorites_list"
    ctx = _Ctx()
    await bot.handle_callback_query(types.SimpleNamespace(callback_query=q, effective_user=_User()), ctx)
    assert q.message.sent and any("המועדפים" in (args[0] if args else "") for args, _ in q.message.sent)
