...
### 9) תמיכה בדיאגרמות בתוך Markdown (Mermaid) בצורה בטוחה
- **הבעיה**: מסמכים טכניים בלי דיאגרמות הרבה פחות ברורים.
- **מה המשתמש רואה**: בלוקים כמו:
  - ```mermaid
    graph TD
      A-->B
    ```
  - מוצגים כדיאגרמה, עם כפתור “הצג מקור” / “הורד SVG/PNG”.
- **איך מממשים בקצרה**: רינדור בצד לקוח עם הגדרות בטוחות (ללא HTML חופשי) + fallback לטקסט.
...
- **חודש+**: (10) Runbook Runner, (12) Config Drift, (9) Mermaid.
