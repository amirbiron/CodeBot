...
```markdown
## ⚙️ לוגיקה טכנית ומקרי קצה (Critical Logic)

כדי למנוע באגים ויזואליים והתנגשויות, יש לממש את הלוגיקה הבאה בקפדנות:

### 1. מניעת הבהוב טעינה (FOUC Prevention)

**הבעיה:** אם נטען את המצב ב-`DOMContentLoaded` (כמו ב-JS רגיל), המשתמש יראה לשבריר שנייה את המצב הרגיל (Classic) ואז המסך "יקפוץ" למצב Focus.

**הפתרון:** סקריפט חוסם (Blocking Script) בראש העמוד.

**המימוש הנדרש:**
יש להוסיף את הסקריפט הבא ישירות בתוך ה-`<head>`, לפני טעינת ה-CSS:

```html
<script>
  // רץ לפני שהדפדפן מצייר את הגוף
  (function() {
    try {
      const savedMode = localStorage.getItem('display_mode');
      const urlParams = new URLSearchParams(window.location.search);
      const urlMode = urlParams.get('display_mode');

      // היררכיה: URL > LocalStorage > Default
      const mode = urlMode || savedMode || 'classic';

      // הוספת הקלאס מיידית למניעת הבהוב
...
### 2. רספונסיביות במצב Power (Mobile Power Mode)

**הבעיה:** מצב Power מבוסס על טבלה רחבה. במובייל זה ייצור גלילה אופקית שבורה.

**הפתרון:** במסכים צרים (< 768px), מצב Power משנה צורה ל-"רשימה דחוסה".

**הגדרת CSS למובייל:**
- **הסתרת עמודות:** הסתרת עמודות File ID ו-Hash במובייל (עדיין נגישות בלחיצה).
- **שבירת שורות:** תאריך וסטטוס יעברו לשורה מתחת לשם הקובץ.
- **כפתורים:** יישארו זמינים וגדולים מספיק למגע (Touch Target 44px).

### 3. אינטראקציה עם חיפוש (Search Behavior)

**התרחיש:** משתמש נמצא במצב Focus (בו ה-ID מוסתר) ומחפש מחרוזת שהיא ID של קובץ.

**ההתנהגות הנדרשת:**
- מנוע החיפוש (Client Side) חייב לסרוק גם שדות מוסתרים (IDs, Tags).
- אם נמצאה התאמה בשדה מוסתר:
  - **אופציה א':** הצגת תווית זמנית "Match found in ID" על הכרטיסייה.
  - **אופציה ב':** הקפצת הודעת Toast: "נמצאו תוצאות בשדות מוסתרים. עבור למצב Power לצפייה" (עם כפתור מעבר).

### 4. קישורים עמוקים (Deep Linking)

**המטרה:** לאפשר שיתוף לינק שפותח את המערכת במצב ספציפי (למשל, לינק למצגת במצב Focus).

**המימוש:**
- תמיכה בפרמטר URL: `?display_mode=focus`.
- אם הפרמטר קיים, הוא דורס את ה-LocalStorage זמנית (לסשן הנוכחי בלבד), אך לא דורס את ההעדפה הקבועה של המשתמש אלא אם הוא בוחר לשמור אותה ידנית.

### 5. הגנה מפני מצבים שבורים (Fallback)

**התרחיש:** עדכון גרסה שינה את שמות הקלאסים, או שה-LocalStorage מכיל זבל (`mode-undefined`).

**הפתרון:**
- **ולידציה בטעינה:** אם הערך ב-Storage אינו אחד מ-`['focus', 'classic', 'power']` → חזור ל-`classic` ומחק את הערך השגוי.
```
