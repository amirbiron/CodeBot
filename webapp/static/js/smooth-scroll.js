...
(function () {
  ...
  class SmoothScrollManager {
    constructor(options = {}) {
      ...
      requestAnimationFrame(() => this.initSettingsUI());
      try {
    ...
    initSettingsUI() {
      ...
      try {
        const modal = document.getElementById("smoothScrollSettingsModal");
        const launcher = document.getElementById(
          "smoothScrollSettingsLauncher",
        );
        if (!modal || !launcher) return;
        const form = modal.querySelector("form");
        if (!form) return;
        const inputs = {
          enabled: form.querySelector('input[name="smooth-enabled"]'),
          duration: form.querySelector('input[name="smooth-duration"]'),
          easing: form.querySelector('select[name="smooth-easing"]'),
          wheel: form.querySelector('input[name="smooth-wheel"]'),
          keyboard: form.querySelector('input[name="smooth-keyboard"]'),
          momentum: form.querySelector('input[name="smooth-momentum"]'),
        };
        const labels = {
          duration: form.querySelector('[data-field="duration-value"]'),
          wheel: form.querySelector('[data-field="wheel-value"]'),
          keyboard: form.querySelector('[data-field="keyboard-value"]'),
          momentum: form.querySelector('[data-field="momentum-value"]'),
        };
        this.settingsRefs = { modal, launcher, form, inputs, labels };
        ...
        if (saveBtn) {
          saveBtn.addEventListener("click", (event) => {
            event.preventDefault();
            this.onSettingsInputChange(true);
            close();
          });
        }
        ...
        if (testBtn) {
          testBtn.addEventListener("click", (event) => {
            event.preventDefault();
            this.runSettingsTest();
          });
        }
        form.addEventListener("input", () => this.onSettingsInputChange(false));
        form.addEventListener("change", () => this.onSettingsInputChange(true));
        if (this.isAndroid || this.debugMode) {
          launcher.hidden = false;
          launcher.setAttribute("aria-hidden", "false");
        }
        this.populateSettingsForm();
        this.settingsInitialized = true;
        document.documentElement.classList.add("smooth-scroll-ui-ready");
      } catch (_) {}
    }
    ...
    populateSettingsForm() {
      ...
      if (!refs) return;
      const { inputs } = refs;
      if (inputs.enabled) inputs.enabled.checked = !!this.config.enabled;
      if (inputs.duration)
        inputs.duration.value = Math.round(this.config.duration || 0);
      if (inputs.easing)
        inputs.easing.value = this.config.easing || "ease-in-out";
      if (inputs.wheel)
        inputs.wheel.value = Number(this.config.wheelSensitivity || 1).toFixed(
          1,
        );
      if (inputs.keyboard)
        inputs.keyboard.value = Number(
          this.config.keyboardSensitivity || 1.5,
        ).toFixed(1);
      if (inputs.momentum)
        inputs.momentum.value = Math.round(
          this.config.androidMomentumAmplifier ||
            this.defaultConfig.androidMomentumAmplifier,
        );
    ...
    collectSettingsValues() {
      const refs = this.settingsRefs;
      if (!refs) return null;
      const { enabled, duration, easing, wheel, keyboard, momentum } =
        refs.inputs;
      return {
        enabled: enabled ? !!enabled.checked : this.config.enabled,
        duration: duration ? Number(duration.value) : this.config.duration,
        easing: easing ? easing.value : this.config.easing,
        wheelSensitivity: wheel
          ? Number(wheel.value)
          : this.config.wheelSensitivity,
        keyboardSensitivity: keyboard
          ? Number(keyboard.value)
          : this.config.keyboardSensitivity,
        androidMomentumAmplifier: momentum
          ? Number(momentum.value)
          : this.config.androidMomentumAmplifier,
      };
    }

    updateSettingsLabels(values) {
      const refs = this.settingsRefs;
      if (!refs) return;
      const source = values || this.collectSettingsValues() || {};
      const formatter = (val, decimals = 0, suffix = "") =>
        typeof val === "number" ? `${val.toFixed(decimals)}${suffix}` : "";
      if (refs.labels.duration)
        refs.labels.duration.textContent = formatter(
          source.duration || this.config.duration,
          0,
          "ms",
        );
      if (refs.labels.wheel)
        refs.labels.wheel.textContent = formatter(
          source.wheelSensitivity || this.config.wheelSensitivity,
          1,
          "x",
        );
      if (refs.labels.keyboard)
        refs.labels.keyboard.textContent = formatter(
          source.keyboardSensitivity || this.config.keyboardSensitivity,
          1,
          "x",
        );
      if (refs.labels.momentum)
        refs.labels.momentum.textContent = formatter(
          source.androidMomentumAmplifier ||
            this.config.androidMomentumAmplifier,
          0,
          "x",
        );
    ...
    onSettingsInputChange(commit) {
      const values = this.collectSettingsValues();
      if (!values) return;
      this.updateSettingsLabels(values);
      this.updateConfig(values, { skipSave: !commit });
      if (values.enabled) this.enable();
      else this.disable();
      if (commit) {
        try {
          this.savePreferences();
    ...
    resetSettingsToDefaults() {
      ...
      this.updateConfig(base);
      if (base.enabled) this.enable();
      else this.disable();
      this.populateSettingsForm();
    }
    ...
    runSettingsTest() {
      try {
        ...
        testTarget.style.top = `${Math.max(200, Math.floor((document.body.scrollHeight || 4000) / 2))}px`;
        testTarget.style.width = "1px";
        testTarget.style.height = "1px";
        testTarget.style.pointerEvents = "none";
        document.body.appendChild(testTarget);
        this.smoothScrollTo(testTarget, {
          callback: () => {
            setTimeout(() => {
