...
(function(){
  ...
  //
  // **``\S`` בשני הקצוות אינו קישוט.** בלעדיו ``a == b == c`` — שרשור
  // השוואות נפוץ בפייתון — היה מזהה את ``" b "`` כמרקר וצובע אותו.
  // הדרישה שהתו הצמוד למפריד לא יהיה רווח מבטלת את כל המשפחה הזו:
  // ``1 == 1 == True``, ``if x == y == z:``. ``a==b==c`` בלי רווחים
  // עדיין נתפס, בדיוק כמו ש-``~~`` מתנהג — ולזה הפתרון הוא גרשיים,
  // שכבר גוברים על המרקר בסדר החלופות.
