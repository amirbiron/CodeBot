...
# נתיב לקובץ רשימת הריפוים של האדמין
_ADMIN_REPOS_PATH = Path(__file__).parent.parent / 'config' / 'admin_repos.yaml'

# Cache לטעינת admin_repos עם TTL (באותה רוח כמו whats_new)
_admin_repos_cache: List[Dict[str, Any]] = []
_admin_repos_cache_time: float = 0
_admin_repos_cache_lock = threading.Lock()
_ADMIN_REPOS_CACHE_TTL = 300  # 5 דקות
...
def _load_admin_repos() -> List[Dict[str, Any]]:
    """
    טוען את רשימת הריפוים של האדמין מקובץ YAML (עם cache).

    מחזיר רשימה ריקה אם הקובץ חסר או פגום – הדשבורד לא אמור ליפול בגלל זה.
    הקריאה לפונקציה אינה בודקת הרשאות; האחריות על כך היא של הקורא.
    """
    global _admin_repos_cache, _admin_repos_cache_time

    now = time.time()

    with _admin_repos_cache_lock:
        if _admin_repos_cache and (now - _admin_repos_cache_time) < _ADMIN_REPOS_CACHE_TTL:
            return list(_admin_repos_cache)

    try:
        if not _ADMIN_REPOS_PATH.exists():
            return []

        with open(_ADMIN_REPOS_PATH, 'r', encoding='utf-8') as f:
            data = yaml.safe_load(f)

        raw_repos = data.get('repos', []) if data else []
        ...
        for repo in raw_repos:
            ...
            processed_repos.append({
                ...
                'icon': str(repo.get('icon', '') or '').strip() or '📦',
                'badge': str(repo.get('badge', '') or '').strip(),
            })

        with _admin_repos_cache_lock:
            _admin_repos_cache = processed_repos
            _admin_repos_cache_time = now

        return list(processed_repos)

    except Exception as e:
        logger.warning(f"Failed to load admin_repos.yaml: {e}")
        return []
