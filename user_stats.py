...
class UserStats:
    ...
    def get_all_time_stats(self):
        ...
        active_today = sum(
            1 for user in self.stats["users"].values() if user["last_seen"] == datetime.now().strftime("%Y-%m-%d")
        )

        return {"total_users": total_users, "active_today": active_today, "active_week": len(self.get_weekly_stats())}
