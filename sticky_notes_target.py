...
# ---------------------------------------------------------------------------
# צבע הפתק — פלטה סגורה, ומזהה יציב במקום hex
# ---------------------------------------------------------------------------

#: הפלטה הסגורה של צבעי הפתקים — **מקור האמת היחיד**, ושלושת הכותבים
#: (``webapp``, ``mcp_server``, ``services``) קוראים ממנה.
#:
#: **מה שנשמר במסד הוא המפתח, לא ה-**\ ``hex``. הנימוק הוא עלות השינוי
#: הבא: החלפת גוון, או הוספת משמעות לצבע ("דחוף"), נוגעת בשורה אחת כאן
#: ואף פתק אינו צריך מיגרציה. שמירת ``hex`` הופכת כל שינוי כזה למיגרציה
#: על כל הפתקים של כל המשתמשים.
#:
#: **הצהוב הקיים הוא צבע בפלטה, ולא גוון היסטורי של אחר.** ``#FFFFCC``
#: היה ברירת המחדל של הפתקים מאז ומתמיד, ולכן כמעט כל פתק שקיים היום
#: נושא אותו. הוא מקבל עיגול משלו בבורר, וממשיך להיות ברירת המחדל —
#: וכך **אף פתק קיים אינו משנה מראה**, ובכל זאת כולם בתוך הפלטה וניתנים
#: לסינון. הוא ו-``yellow_light`` הם שני גוונים קרובים אך נפרדים, בדיוק
#: כמו שני צבעים אחרים.
#:
#: ``legacy`` הוא מה שיחסוך את המיגרציה **הבאה**: גוונים שמזהה נשא
#: בעבר, שממשיכים להתמפות אליו. הוא ריק היום בכוונה — אין כאן גוון
#: שהוחלף. **מי שמחליף גוון בעתיד מוסיף את הישן לשורה שלו**, ובזה
#: המיגרציה נגמרת.
#:
#: הסדר כאן הוא סדר התצוגה בבורר — ``dict`` בפייתון שומר סדר הכנסה,
#: ו-:data:`NOTE_COLOR_ORDER` נגזר ממנו כדי שלא תיווצר רשימה שנייה
#: שיכולה להיסחף. הצהוב הקיים ראשון כי הוא ברירת המחדל.
NOTE_COLORS: Dict[str, Dict[str, Any]] = {
...
#: סדר התצוגה בבורר, נגזר מ-:data:`NOTE_COLORS` ולא מוקלד שנית.
NOTE_COLOR_ORDER: Tuple[str, ...] = tuple(NOTE_COLORS)

#: ברירת המחדל לפתק חדש, וגם הנפילה כשהערך שהגיע אינו ניתן לפענוח.
#: **הצהוב הקיים** — פתק חדש נולד בדיוק בצבע שפתקים נולדו בו תמיד.
DEFAULT_NOTE_COLOR_ID = "yellow"

#: הצורות שהתקבלו כ-``hex`` מאז ומתמיד. **הטווח ``3,8`` נשמר בכוונה**
#: ואינו מצומצם כאן: ``mcp_server/handlers`` קיבל אותו, ולכן צמצום היה
#: דוחה ערכים שכבר יושבים במסד ומופיעים בגיבויים.
_HEX_COLOR_RE = re.compile(r"^#[0-9A-Fa-f]{3,8}$")


def normalize_color_hex(value: Any) -> str:
    """``hex`` בצורה קנונית אחת, או ``""`` כשהערך אינו ``hex`` כלל.

    **הנורמליזציה היא כל העניין, לא קוסמטיקה.** ``#FFFFCC``, ``#ffffcc``
    ו-``#FFC`` הם אותו צבע בדיוק על המסך, ובלי התכנסות לצורה אחת הם
    שלושה ערכים שונים במסד — כלומר סינון לפי צבע מחזיר שליש מהפתקים,
    בלי שגיאה ובלי סימן. שלוש ההמרות:

    * אותיות קטנות.
    * ``#rgb`` / ``#rgba`` מורחבים לכפילת כל ספרה, כמו שהדפדפן עושה.
    * ``alpha`` אטום (``ff``) בסוף נחתך — ``#ffffbaff`` הוא ``#ffffba``.

    **חמש ושבע ספרות נשארות כמות שהן** (בלי אותיות גדולות): הן אינן צורת
    ``hex`` חוקית ב-CSS ולכן הדפדפן מתעלם מהן ממילא, אבל הרג'קס שקדם
    לכאן קיבל אותן — ודחייה שלהן כאן הייתה מוחקת ערך קיים במסד במקום
    להשאירו ``legacy``.

    **רק מחרוזת היא ``hex``.** גוף JSON יכול לשאת מספר, רשימה או ``None``,
    ו-``.strip()`` על כל אחד מהם זורק — כלומר 500 במקום 400, לפני
    שהוולידציה הספיקה לומר מה לא תקין.
    """
    if not isinstance(value, str):
        return ""
    raw = value.strip()
    ...
    if len(digits) == 8 and digits.endswith("ff"):
        digits = digits[:6]
    return "#" + digits


#: ``hex`` מנורמל ← מזהה בפלטה. נבנה **פעם אחת** מ-:data:`NOTE_COLORS`,
#: כדי שהגוון הקנוני והגוונים ההיסטוריים יגיעו שניהם מאותה שורה. טבלה
#: שנכתבת ביד לצד הפלטה היא בדיוק המקום שבו שתיהן נסחפות.
...
for _color_id, _color_spec in NOTE_COLORS.items():
    for _variant in (_color_spec["hex"],) + tuple(_color_spec.get("legacy") or ()):
        _normalized = normalize_color_hex(_variant)
        if _normalized:
            _HEX_TO_COLOR_ID[_normalized] = _color_id
del _color_id, _color_spec, _variant, _normalized


def resolve_note_color(value: Any, *, default: Optional[str] = DEFAULT_NOTE_COLOR_ID) -> str:
    """**הצורה שנשמרת במסד** — מזהה פלטה, או ``hex`` legacy מנורמל.

    זו הפונקציה היחידה שמחליטה מה נכתב לשדה ``color``, וכל שלושת
    הכותבים עוברים דרכה. הקלט סובלני בכוונה — מזהה מהפלטה **או** ``hex``
    חופשי — כי סוכן דרך ה-MCP כותב ``hex``, בעוד שהבורר בממשק כותב מזהה,
    ושניהם צריכים להגיע לאותה צורה מאוחסנת.

    סדר ההכרעה:

    1. מזהה שקיים בפלטה ← נשמר כמות שהוא.
    2. ``hex`` שמתנרמל לגוון של הפלטה (קנוני **או** ``legacy``) ← נשמר
       כמזהה שלו. זה מה שהופך את ``#FFFFCC`` של פתק ישן ל-``yellow_light``
       בלי סקריפט.
    3. ``hex`` אחר ← נשמר כמות שהוא, מנורמל. הוא ``legacy``, והוא לא
       נמחק ולא מוחלף בכוח.
    4. כל השאר ← ``default``.

    ``default=None`` מחזיר ``""`` — "לא הצלחתי לפענח". **מה עושים עם
    התשובה הזו הוא החלטה של הקורא, לא של הפונקציה**, ושני הקוראים
    מחליטים אחרת בכוונה:

    * ``mcp_server/handlers`` **דוחה** בשגיאה עם רשימת המזהים התקינים.
      סוכן שמקבל ``ok`` על צבע שלא הוחל מדווח למשתמש דבר לא נכון, והוא
      גם היחיד שיכול לתקן בניסיון הבא אם אומרים לו מה חוקי.
    * ``webapp/sticky_notes_api`` **שומט** בעדכון. הבורר שם שולח רק
      מזהים שעברו בדיקה מול הפלטה בצד הלקוח, ולכן ערך פסול אינו קלט
      משתמש אלא באג בקוד שלנו — ודריסת הצבע הקיים בברירת מחדל הייתה
      הופכת אותו לאובדן נתונים במקום ל-no-op.
    """
    if isinstance(value, str):
        candidate = value.strip().lower()
        if candidate in NOTE_COLORS:
            return candidate
        as_hex = normalize_color_hex(value)
        if as_hex:
            return _HEX_TO_COLOR_ID.get(as_hex, as_hex)
    return default or ""


def note_color_id(stored: Any) -> str:
    """המזהה בפלטה של ערך מאוחסן, או ``""`` לצבע ``legacy``.

    זה מה שהבורר בממשק מסמן, וזה מה שסינון לפי צבע ישווה אליו. ``""``
    אינו כשל — הוא התשובה הנכונה לפתק שצבעו אינו בפלטה.
    """
    if not isinstance(stored, str):
        return ""
    candidate = stored.strip().lower()
    if candidate in NOTE_COLORS:
        return candidate
...
def note_color_hex(stored: Any) -> str:
    """ה-``hex`` שמוצג בפועל — **נגזר, ולעולם לא מאוחסן לצידו**.

    שני שדות שחייבים להישאר מסונכרנים הם שני שדות שיסטו; לכן במסמך יש
    ערך אחד, וזו הפונקציה שגוזרת ממנו את מה שהדפדפן צובע. פתק ``legacy``
    מחזיר את ה-``hex`` של עצמו, ולכן ממשיך להיראות בדיוק כפי שנראה.

    ערך שאינו ניתן לפענוח כלל מחזיר את גוון ברירת המחדל ולא ``""``:
    הצרכן היחיד הוא ``style.backgroundColor``, ומחרוזת ריקה שם משאירה
    פתק שקוף על רקע העמוד — כלומר טקסט על גבי קוד.
    """
    # **דרך ``note_color_id`` ולא במקביל לה.** הגרסה הראשונה בדקה כאן
    # "האם זה מזהה?" ואם לא — החזירה את ה-``hex`` כמות שהוא. התוצאה:
    # ``#FFFFCC`` קיבל ``id="yellow_light"`` אבל ``hex="#ffffcc"``, כלומר
    # הבורר סימן "צהוב" על פתק בגוון שאינו הצהוב של הפלטה. שני חלקים
    # שחולקים על אותה שאלה — נתפס בבדיקה, לא בעין.
    palette_id = note_color_id(stored)
    if palette_id:
        return str(NOTE_COLORS[palette_id]["hex"])
    as_hex = normalize_color_hex(stored)
    if as_hex:
        return as_hex
    return str(NOTE_COLORS[DEFAULT_NOTE_COLOR_ID]["hex"])
