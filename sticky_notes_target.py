...
#:
#: ``legacy`` הוא מה שהופך את ההבטחה הזו לאמיתית ולא להצהרה: גוונים
#: שהמזהה נשא בעבר, שממשיכים להתמפות אליו. ``#FFFFCC`` נמצא שם כי הוא
#: היה ברירת המחדל של הפתקים מאז ומתמיד — בלעדיו כל פתק שקיים היום היה
#: יוצא מחוץ לפלטה, כלומר ``legacy`` שאי אפשר לסנן לפיו. **מי שמחליף
#: גוון בעתיד מוסיף את הישן לשורה הזו**, ובזה המיגרציה נגמרת.
#:
...
DEFAULT_NOTE_COLOR_ID = "yellow_light"
