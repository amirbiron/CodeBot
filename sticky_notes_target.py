...
def resolve_note_color(value: Any, *, default: Optional[str] = DEFAULT_NOTE_COLOR_ID) -> str:
    ...
    ``default=None`` מחזיר ``""`` ומשמש בעדכון, שם ערך פסול צריך
    **להישמט** ולא לדרוס את הצבע הקיים בברירת מחדל. זו בדיוק ההבחנה
    שכבר קיימת ב-``mcp_server/handlers``: ביצירה נופלים לברירת המחדל,
    בעדכון שומטים.
    """
