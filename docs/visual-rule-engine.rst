...
==========================================
:summary: מנוע כללים ויזואלי ליצירת התראות מורכבות מהממשק בלי לכתוב קוד: זרימת ההחלטה, מסך הכללים, יצירה והפעלה, וסכמת ה-JSON של כלל.

.. note::
   פיצ'ר זה זמין רק למשתמשי אדמין (``ADMIN_USER_IDS``).
   ראו :doc:`security` להגדרת הרשאות.

מדריך קצר (Quick Start)
-----------------------

זה המדריך הקצר למי שרוצה “להתחיל לעבוד” בלי לצלול לכל הפרטים.

1. **כניסה למסך**

   - היכנסו ל־WebApp כמשתמש אדמין
   - פתחו: **Admin → Visual Rules** (``/admin/rules``)

2. **בניית תנאים (IF)**

   - בבונה הכללים לחצו על אחד הכפתורים:
     - **+ תנאי**
     - **+ קבוצת AND**
     - **+ קבוצת OR**
     - **🚫 קבוצת NOT** (היפוך של תנאי/קבוצה, מקבל ילד אחד)
   - בכל תנאי בחרו:
     - **שדה (Field)** מתוך הרשימה
     - **אופרטור (Operator)** (למשל ``eq``/``contains``/``regex``)
     - **ערך (Value)**
   - אפשר לגרור ולסדר מחדש בלוקים בעזרת ידית הגרירה (האייקון בצד הבלוק).

3. **בניית פעולות (THEN)**

   - לחצו **+ פעולה**
   - בחרו סוג פעולה (למשל: ``suppress`` / ``send_alert`` / ``create_github_issue`` / ``webhook``)
   - הגדירו חומרה (``info``/``warning``/``critical``) ושדות נוספים לפי סוג הפעולה

4. **בדיקה בלי “לשבור פרודקשן”**

   - לחצו **בדוק כלל**
   - הדביקו JSON לדוגמה בחלון הבדיקה
   - הריצו בדיקה וראו האם הייתה התאמה ואילו פעולות *היו* מתבצעות (Dry-Run)

5. **שמירה והפעלה**

   - מלאו **שם כלל** (מומלץ גם **תיאור**)
   - ודאו שהמתג **כלל פעיל** מסומן אם אתם רוצים שירוץ בפועל
   - לחצו **שמור כלל**

6. **עריכה/מחיקה**

   - כדי לערוך כלל קיים: לחצו **ערוך** בטבלת “כללים קיימים”, בצעו שינויים ואז **שמור כלל**
   - כדי למחוק: לחצו **מחק**

.. important::
   אם פעולה היא ``suppress`` ההתראה *לא תישלח בכלל* (ההערכה מתבצעת לפני השליחה לערוצים).

שני מתכונים נפוצים
^^^^^^^^^^^^^^^^^^

מתכון 1: השתקת התראות בסביבת development
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- **IF**: ``environment eq development``
- **THEN**: ``suppress``
- **מטרה**: להפחית רעש מהתראות בדיקות/לוקאל.

מתכון 2: פתיחת GitHub Issue על שגיאת Sentry חדשה
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- **IF**:
  - ``alert_type eq sentry_issue``
  - ``is_new_error eq True``
- **THEN**: ``create_github_issue`` עם ``title_template`` / ``params.title`` (למשל: ``[Sentry] {{summary}}``)
- **מטרה**: לא לפספס שגיאות חדשות, בלי לפתוח כפילויות (המנגנון מונע Issue כפול לפי כותרת).
