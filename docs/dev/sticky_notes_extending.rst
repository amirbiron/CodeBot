...
==================================
:summary: המלכודות החוזרות של הפתקים הדביקים: יעד יחיד ב-build_note_target, מכסה fail-closed, כוונה מול תצוגה בגודל ובמיקום, flush לפני פעולה הרסנית, אינדקס ממוספר, נורמליזציה בשני הקצוות, יתומים בקריאה, ועדכון עמוד המשתמש.
...
כוונה לעומת תצוגה: מה נשמר, ומה נגזר בזמן ריצה
------------------------------------------------
מיקום וגודל של פתק נשמרים בפיקסלים מוחלטים (``position_x``, ``position_y``, ``width``, ``height``). ערך מוחלט תקף רק למסך שהוא נמדד בו, ולכן פתק שנוצר על מסך רחב חוצה את הגבול על מסך צר. שלושת התסמינים שנצפו — סיבוב מכשיר, כניסה לאותו לוח/קובץ ממכשיר צר יותר, ופתיחת פתק מהתראת פוש על מכשיר אחר — הם **שורש אחד**: קריאת ערך מוחלט על מסך שהוא לא נשמר עבורו. תיקון של כל תסמין בנפרד היה שלושה טלאים לאותה בעיה.

לכן יש כאן שתי שכבות, ולא אחת:

- **הכוונה** — ``entry.data.position`` ו-``entry.data.size``. מה שהמשתמש קבע, וזה גם מה שיושב במסד.
- **המוצג** — ``el.style``. נגזר בזמן ריצה מהכוונה מול גבולות המסך הנוכחיים, ב-``_fitToFrame``.

ההקטנה וההצמדה חיות בשכבה השנייה **בלבד**. אין להן נתיב לשכבה הראשונה — וזו לא הידור, זו כל התכונה: בלי ההפרדה, ביקור אחד בטלפון היה מקבע את הגודל של הטלפון לתמיד.

.. important::
   **גודל לא נמדד ממסלול שמסתיים בשמירה.** ``_notePayloadFromEl`` לוקח את הגודל מהכוונה ולא מ-``getBoundingClientRect``, כי המדידה מחזירה את המוצג. שלוש הקטנות שונות נסגרות כאן בבת אחת, ושתיים מהן כבר קרו בפועל:

   - ההקטנה למסך צר — הנושא של הסעיף הזה.
   - ``height: auto !important`` של פתק ממוזער. המדידה מחזירה את גובה הכותרת בלבד, והגובה השמור נפל לרצפה.
   - **``transform`` בזמן אנימציה.** ``noteAppear`` מתחיל ב-``scale(.9)``, ו-``getBoundingClientRect`` מחזיר את הקופסה **אחרי** הטרנספורם. מדידה בזמן שהאנימציה רצה החזירה 90% מהרוחב, והמסלול שכתב אותה בחזרה ל-``style`` הקטין את הפתק בכל טעינה — כפול, כי הוא רץ פעמיים. נמדד בכרומיום: פתק של 700px הוצג ב-567px, שהם ‎0.9²‎.

   המכנה המשותף אינו "המדידה שגויה" אלא **המדידה מחזירה את המוצג, וזה מה שהיא אמורה להחזיר.** לכן היא אינה מקור לגיטימי לערך שנשמר — לא עכשיו, ולא כשיתווסף החיתוך הבא ב-CSS.

**מי כן מעדכן את הכוונה, ואיפה.** גרירה מהפינה, ב-``_enableResize``, ושם בלבד. הפרדה בין "שינוי גודל שנבע מהמשתמש" ל"שינוי שנבע מהתאמת מסך" היא תנאי הכרחי; בלעדיה שתי הפעולות נראות זהות בקוד, ואין דרך לדעת מה לשמור.

**המיקום דורש את אותה הפרדה, בדרך אחרת.** את המיקום אי אפשר לקחת מהכוונה בזמן שמירה, כי מעבר בין מרחבי קואורדינטות (משטח מול מסמך) נגזר ממדידה — ראו ``_positionOrigin``. במקום זה, ההיסט שההצמדה הוסיפה נרשם ב-``entry.fitOff`` ומוחזר בשמירה. בלעדיו כל פעולה שנעשית בזמן שהפתק מוצמד — נעיצה, מעבר ``surface``↔``screen`` — הייתה כותבת את ההצמדה הזמנית ככוונה. גרירה מאפסת את ההיסט: שם המיקום הוא באמת בחירה של המשתמש, גם כשהיא נעצרה בגבול.

.. note::
   **``_applyPositionMode`` קובע את ההיסט, ``_reflowWithinViewport`` מוסיף לו.** הראשונה מחשבת מהכוונה מאפס; השנייה מצמידה מעל מצב שכבר הותאם, ולכן קביעה בה הייתה מוחקת את ההיסט הקודם. שתי סמנטיקות שונות לאותו שדה, ושתיהן נדרשות.

**גבול שלא נמדד אינו גבול צר.** ``_frameBounds`` מחזירה ``w = 0`` כשאי אפשר למדוד את הקונטיינר, ואז לא מקטינים ולא מצמידים כלום. זו אותה הבחנה שחוזרת בעמוד הזה: כשל מדידה נבדל מ"אין מקום", בדיוק כמו שכשל שאילתה נבדל מ"אין יתומים".

**איפה בכלל הגבול.** ``screen`` הוא ``position: fixed`` ולכן שני הצירים חסומים באזור התצוגה. ``surface`` ו-``anchored`` הם ``absolute`` בתוך הקונטיינר: הרוחב חסום, אבל הגובה לא — ``_applySurfaceExtent`` מאריך את המשטח עד הפתק הרחוק ביותר, ולכן פתק גבוה נגיש בגלילה ואין סיבה להקטין אותו. ילד ``absolute`` בלי אב ממוקם נמדד מול ``documentElement.clientWidth`` ולא מול רוחב התוכן של ה-``body``; מדידה של האחד והצמדה מול השני הייתה משאירה בדיוק את הפינה מחוץ למסך.

**הרישום לסיבוב מסך.** ``window.orientationchange`` מסומן deprecated ב-MDN, שמפנה ל-`change של ScreenOrientation <https://developer.mozilla.org/en-US/docs/Web/API/ScreenOrientation/change_event>`_. ``resize`` (וגם ``visualViewport.resize``) מכסה את רוב הסיבובים ממילא, ולכן שלושתם מחוברים לאותו חישוב אידמפוטנטי. הנקודה החשובה אינה איזה אירוע אלא **מה הוא מפעיל**: קודם רץ שם ``_reflowWithinViewport``, שמדלג על ``is-pinned`` — כלומר סיבוב מכשיר לא נגע בפתקי לוח, בפתקי ריפו ובפתקים מעוגנים בכלל. ``_refitAll`` מכסה את שניהם.

תיאור ההתנהגות למשתמש נמצא ב-:doc:`/user/sticky_notes`.
