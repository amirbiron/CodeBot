...
``screen`` הוא היחיד שמוצמד בשני הצירים, ולכן הוא היחיד שעובר ב-``_fitAndClampToViewport`` — הפונקציה המשותפת לענף הצף ול-``_reflowWithinViewport``. שני המצבים האחרים משתמשים ב-``_resolveDisplaySize`` ומצמידים X בעצמם.

.. warning::
   ``is-pinned`` מסמן **גם** ``surface`` וגם ``anchored``, ולכן ``_reflowWithinViewport`` מדלג על שניהם. מי שיוסיף מצב מיקום חדש ויסמן אותו ב-``is-pinned`` יקבל בירושה גם את הדילוג הזה: הצמצום שלו חייב לחיות בענף שלו ב-``_applyPositionMode``, שאליו ``_reapplyPinnedLayout`` מגיע בשינוי גבולות. המצב ``anchored`` שכח את זה ונשאר בלי מדיניות גודל בכלל — פתק ברוחב 600 גלש במסך 320 כבר בטעינה, לא רק אחרי סיבוב.

**שינוי גבולות חייב מסלול משלו לפתקים הנעוצים.** ``_reflowWithinViewport`` מדלג על ``is-pinned`` בכוונה: המיקום של פתק משטח נמדד במרחב המשטח ולא במסך, והצמדה לפי ה-viewport הייתה מזיזה אותו למקום שגוי. לכן ``resize`` קורא גם ל-``_reapplyPinnedLayout``, שמריץ ``_applyPositionMode`` על הפתקים הנעוצים — הפונקציה היחידה שמצמצמת פתק משטח לרוחב המשטח. בלעדיה לוח שנטען במסך רחב ואז סובב נשאר עם פתקים ברוחב הישן, גולשים מעבר לקצה עם הידית מחוץ למסך. במאזין של ``visualViewport`` אין צורך באותה קריאה: שם משתנה החלון הנראה (מקלדת, פינץ'-זום) ולא רוחב המשטח, שממנו נגזר הצמצום.
