...
**גבול בלוק סוגר לפי ההזחה של עצמו — כלל אחד, בלי ענף לכל סוג.** ``_closeListsAbove`` היא כל התשובה: היא מקבלת הזחה ומוציאה מהמחסנית כל פריט שההזחה כבר אינה בתוכו. שורת רשימה, כותרת, ציטוט, קו מפריד, טבלה, שורת טקסט ושורת גדר — כולן עוברות דרכה, ולכן בלוק בעמודה 0 סוגר את הרשימה ובלוק שמוזח לתוך הפריט נשאר בתוכו.
...
.. note::
   **הפרדיקט הוא** ``!inFence``\ **, לא** ``!inCode``. ההבחנה היא כל התיקון: כשרואים את שורת הגדר **הפותחת** הדגל עדיין ``false``, והוא מתהפך רק בסוף הענף שמרנדר אותה — ולכן הפותחת נכנסת לכלל בלי ענף מיוחד, בעוד שורת הסגירה ושורות התוכן רואות ``true`` ואינן נוגעות במחסנית. זה גם נכון סמנטית: הפותחת היא הגבול שקובע הכלה, והסגירה שייכת לאותו בלוק. ``!inCode`` היה מוציא גם את הפותחת, כלומר משאיר את הבאג המקורי — וזו בדיוק המוטציה שמכסה את השורה הזו בטסטים.
