...
הצבע נשמר כ**מזהה** מהפלטה (``yellow_light``) ולא כ-``hex``. השיקול הוא עלות השינוי הבא ולא אסתטיקה: החלפת גוון, או הוספת משמעות לצבע ("דחוף"), נוגעת בשורה אחת ב-``NOTE_COLORS`` ואף פתק אינו צריך מיגרציה. עם ``hex`` שמור, כל שינוי כזה הוא כתיבה על כל הפתקים של כל המשתמשים.
...
.. important::
   **‏``note_color_hex`` עוברת דרך ``note_color_id``, ולא במקביל לה.** הגרסה הראשונה שאלה "האם זה מזהה?" ואם לא — החזירה את ה-``hex`` כמות שהוא. התוצאה: ``#FFFFCC`` קיבל ``id="yellow_light"`` אבל ``hex="#ffffcc"``, כלומר הבורר סימן "צהוב" על פתק בגוון שאינו הצהוב של הפלטה. זה אותו דפוס שכבר תועד כאן על מפריד הטבלה ועל יצרן ההזחה: **שני חלקים שחולקים על אותה שאלה.** נתפס בבדיקה ולא בעין.

``legacy`` הוא מה שהופך את ההבטחה הזו לאמיתית ולא להצהרה. כל מזהה נושא את הגוונים שהוא סחב בעבר, וכולם מתמפים אליו. ``#FFFFCC`` יושב שם כי הוא היה ברירת המחדל של הפתקים מאז ומתמיד — בלעדיו כל פתק שקיים היום היה נשאר ``legacy``, כלומר מחוץ לפלטה ולא ניתן לסינון לפי צבע. **מי שמחליף גוון בעתיד מוסיף את הישן לאותה שורה, ובזה המיגרציה נגמרת.**
...
.. important::
   **ביצירה נופלים לברירת המחדל, בעדכון שומטים** — ``default=None``. ההבחנה אינה סגנונית: ביצירה אין צבע קודם, ואילו בעדכון יש צבע שהמשתמש בחר בכוונה, וכתיבת ברירת מחדל עליו הייתה מוחקת בחירה שאיש לא ביקש לשנות. ההבחנה קדמה לפלטה ונשמרה דרכה.

``color`` בתשובת ה-API הוא ``hex``, ולא מה שמאוחסן
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

התשובה נושאת שני שדות: ``color`` תמיד ``hex`` חוקי, ו-``color_id`` שהוא המזהה או ``""`` ל-``legacy``. ``color`` נשאר ``hex`` כי הצרכן היחיד שלו הוא ``style.backgroundColor``, ולקוח שנטען מקאש ישן היה מכניס לשם ``yellow_light`` — ערך CSS לא חוקי, כלומר פתק שקוף על גבי הקוד שמתחתיו. אותה סיבה בדיוק היא שהנפילה ב-``note_color_hex`` היא לגוון ברירת המחדל ולא ל-``""``.
...
.. warning::
   **וההצלבה חייבת לכסות גם את ``legacy``.** הגרסה הראשונה שלה השוותה גוון ותווית בלבד ועברה, בזמן שה-JS בנה את מפת ה-``hex`` מהגוונים הקנוניים בלבד ודילג על ההיסטוריים. התוצאה: ``#FFFFCC`` זוהה בשרת ולא בלקוח, כלומר פתק של משתמש קיים פתח בורר בלי שום עיגול מסומן. **שומר שבודק חלק מהשורה שומר על חלק מהתשובה.**
...
* **טקסט על ה-swatch** — הדיו ``#2b3f6a`` מול גווני הפלטה יוצא 8.14:1 (ורוד, הגרוע) עד 10.02:1 (צהוב), כלומר מעל הסף המחמיר של WCAG AAA. לכן התווית יושבת **על** הצבע ואינה צריכה רצועה משלה.
...
.. warning::
   **הטוקן היחיד בפתק שתלוי ברקע הוא המרקר, וכעת יש לו שישה רקעים במקום אחד.** כל שאר הצבעים בקובץ שקופים או נגזרים מהדיו (``transparent``, ``rgba(0,0,0,…)``, ``color-mix(… , transparent)``) ולכן מסתגלים לכל גוון מאליהם; ``--sticky-mark-bg``/``--sticky-mark-line`` הם צבעים אטומים שההערה בקוד מתעדת שכוילו מול הנייר הצהוב.
   ...
   מי שיוסיף כאן צבע שביעי — או ישנה את גוון הוורוד — מודד את המספר הזה, ואם המרקר נבלע התשובה היא **טוקן מרקר פר-צבע** ולא כיול מחדש של הערך המשותף.
...
הקריאה מנורמלת בעצמה — ``note_color_id`` מקפל ``#FFFFCC`` ל-``yellow_light`` בכל שליפה — ולכן התצוגה והבורר נכונים גם בלי להריץ דבר. מה שהיא **אינה** פותרת הוא השאילתה: סינון לפי צבע מחפש ערך אחד במסד, ומסד שמחזיק חלק מהפתקים כ-``#ffffcc`` וחלק כ-``yellow_light`` מחזיר חלק מהתשובה.

לכן ``scripts/migrate_note_colors.py`` מיישר את מה שקיים, והנורמליזציה בקריאה מכסה את מה שנכתב בין ההרצה לפריסה ואת גיבויים שמשוחזרים אחריה. הסקריפט הוא דוח בלבד כברירת מחדל, בתבנית ``migrate_note_boards.py``.
