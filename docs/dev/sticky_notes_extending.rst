...
**ומצב הפתיחה חייב לשרוד רינדור מחדש.** ``<details>`` נייטיבי מחזיק את מצבו ב-DOM בלבד, והתצוגה נבנית מאפס בכל ``_syncTaskView`` — יציאה מעריכה, ובעיקר ``_applyServerContent`` שרץ אחרי סימון צ'קבוקס. בלי שחזור, בלוק שנפתח היה נסגר תחת האצבע בדיוק ברגע שמסמנים משימה שבתוכו. המפתח הוא ההיסט של שורת הפתיחה — אותה זהות שהמנוע כבר מתחזק, ושסימון צ'קבוקס אינו מזיז, כי ``[ ]`` ו-``[x]`` באותו אורך.
...
``.markdown-details``, ``.markdown-summary`` ו-``.details-content`` מוגדרות ב-``markdown-enhanced.css`` שנטען ב-``base.html``, ולכן גם כאן אין פלטה חדשה. וגם המלכודת זהה — ובאמת אותה שורה: ``.markdown-details, .admonition { margin: 0.75rem -0.5rem; }`` בכלל המובייל תופסת את שתי המחלקות יחד.
