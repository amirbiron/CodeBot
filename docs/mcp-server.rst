...
``.rst`` תחת ``amir-bug-patterns``, מחזירים::
   ...
   {"ok": false, "error": "suffix_not_allowed",
...
.. note::
   ...
   **הגבול נבדק כיחידת נתיב, והנרמול קורה אחריו.** ``docsecret/x`` אינו תחת
   ``docs/`` אף שהוא מתחיל באותם תווים, ו-``docs/../secrets`` נדחה כי הוא
   נפתר אל מחוץ לשורש. מנגד ``bugbot-rules/../README.md`` **מתקבל** — הוא
   נפתר אל תוך הריפו, ו-``..`` אינו מילה אסורה אלא נרמול שנבדק לפי **לאן**
   הוא מגיע.
...
``too_many_sections``
   קובץ ה-Markdown חוצה את תקרת הכותרות של הפארסר (50,000). מסלול ה-RST
   נקרא **בלי תקרה**, בכוונה ובהצהרה — תקרה לו היא שינוי התנהגות על מסלול
   חי, והיא אישו נפרד.
