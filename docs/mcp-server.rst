...
.. list-table::
   ...
   * - ``codekeeper_search_repo``
     - חיפוש טקסט בריפו (קטעים קצרים עם path+line, עם תקרות).
       ...
       ``total`` הוא כמה מופעים יש בריפו ולא כמה הוחזרו, וקוד חיצוני מוחרג
       מהחיפוש ומהספירה כברירת מחדל — ראו :ref:`mcp-search-total`
...
נעצרה בתקרה), ``count_failed``, ``matches_per_file`` (התקרה של ההתאמות
לכל קובץ), ``policy_filtered`` (שורה הוסרה במדיניות הסודות), ``timeout``,
``output_limit`` ו-``process_killed``. כשהספירה היא זו שנקטעה, **הסיבה שלה גוברת** — כי
...
.. important::
   ...
   **לא נוצרת גרסה, ולכן התיאור הקודם אינו נשמר בשום מקום.**
   ...
   השינוי הוא ``$set`` על מסמך הגרסה האחרונה. שלוש נגזרות שחשוב להכיר:
...
**קוד חיצוני הוא רשימה אחרת לגמרי, ולא להתבלבל.** סוד חסום **גם בקריאה**;
``node_modules`` רק אינו נכלל בחיפוש כברירת מחדל, וממשיך להיקרא דרך
``codekeeper_get_repo_file``. ראו :ref:`mcp-search-total`.
...
- **קוד חיצוני** (``node_modules`` בכל עומק), כברירת מחדל. ``include_vendored=true``
  מחזיר אותו לחיפוש ולספירה. קבצים כאלה ממשיכים להיקרא כרגיל דרך
  ``codekeeper_get_repo_file`` — ההחרגה היא של החיפוש בלבד.
...
**ההתאמה אינה רגישה לרישיות**, כמו ``git grep -i`` ב-``codekeeper_search_repo``.
ההשוואה היא ב-``casefold``, אותה בחירה כמו ``symbol=`` באאוטליין.
