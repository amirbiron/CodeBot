...
- ``lines`` — מאוסף ``repo_files``, שהאינדקסר (``services/code_indexer.py``)
  ממלא. שאילתה אחת לעמוד, נשענת על האינדקס ``(repo_name, path)`` שהשירות
  מוודא בעלייה, וחסומה בגודל העמוד ולא בגודל הריפו. **git אינו יכול לספק ספירת שורות בלי
  לקרוא כל בלוב בנפרד**, וזו עלות שלא משלמים כאן.
