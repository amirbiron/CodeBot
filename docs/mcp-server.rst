...
שתי התשובות נושאות גם ``line`` — **מספר השורה שגרמה לסירוב**, 1-מבוסס, כפי
שהפארסר עצמו חישב אותו. זה מה שהופך את הסירוב לפעולה: ``inconsistent_line_endings``
בלי מספר אומר "יש איפשהו ``\r`` בקובץ של 4,000 שורות". השדה מופיע רק כשיש
מספר, ולא כ-``null`` — כמו ``suggestions_truncated`` ו-``remaining_chars``
באותו כלי.
