...
- ``size`` — מ-``git ls-tree -r -l``, כלומר גודל הבלוב בבייטים. **אפס עלות
  ...
  התוכן שלו חי בריפו אחר. הנתיב עצמו מופיע כרגיל בשתי הרשימות — ``paths``
  זהה עם הדגל ובלעדיו, גם כשיש תת-מודולים בעץ.
- ``lines`` — מאוסף ``repo_files``, שהאינדקסר (``services/code_indexer.py``)
  ממלא. שאילתה אחת לעמוד, נשענת על האינדקס הייחודי ``(repo_name, path)``
  וחסומה בגודל העמוד ולא בגודל הריפו. **git אינו יכול לספק ספירת שורות בלי
