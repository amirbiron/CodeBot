...
לכן כל עדכון שנוגע ב-``content`` שומר את הגוף הקודם באוסף
``sticky_note_versions`` — **לפני** הדריסה. הקריאה היא read-modify-write,
ולכן הסדר אינו הפיך: ``codekeeper_note_str_replace`` נוסף **אחרי**
ההיסטוריה ולא לפניה, שאם לא כן היה עוד מסלול שדורס בלי רשת.
