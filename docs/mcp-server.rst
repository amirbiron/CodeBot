...
כל כלי, ``report_missing`` רושמת כלי וירטואלי, ו-``enable_conversation_id``
מזריקה ארגומנט נוסף ומוסיפה הנחיה לתוצאת הכלי. שלושתן משנות את מה שהשרת
מפרסם ללקוח, ולכן שלושתן כבויות — הסכימות אחרי האינסטרומנטציה זהות לאלה
שלפניה. גם ``capture_exception_code_variables`` כבוי במפורש: הוא לוכד משתנים
מקומיים, ובשרת הזה המשתנים המקומיים מחזיקים תוכן קבצים.
...
פועלת (עם אזהרה בלוג), ובסביבת פיתוח (``ENVIRONMENT``/``ENV`` שאינו
``production``) העלייה נכשלת ברעש, כדי ש"אין אירועים" לא ייראה כמו "הכל
תקין". הטוקן לעולם אינו בקוד ואינו בריפו.
