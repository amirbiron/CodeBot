...
.. note::
   ...
   **אין כאן ``truncated``, ובכוונה.** ב-``codekeeper_list_repo_tree`` המילה
   ...
   באאוטליין אין חיתוך כזה בכלל: אורך השם חסום ב-200 תווים, ולכן רשומה
   שוקלת לכל היותר כ-250 בתים ועמוד מקסימלי הוא 500 × 250 = 123,000 —
   פחות ממחצית תקציב הפלט. זו אריתמטיקה ולא תקווה, ויש עליה טסט.
...
**שם ארוך מ-200 תווים מקוצץ ומסומן ב-**\ ``…``. השם הארוך ביותר בריפו הזה
הוא 81 תווים, אז המגבלה אינה נוגעת בקוד אמיתי — היא קיימת כדי שגודל הרשומה
יהיה חסום, וזה מה שמאפשר להוכיח שעמוד נכנס בתקציב במקום לחתוך בזמן ריצה.
...
.. list-table::
   ...
   * - מצב
     - התשובה
   * - קובץ שאינו פייתון
     - ``{"status": "no_outline", "reason": "unsupported_language"}``
   * - תחביר שבור, פייתון 2, קידוד פגום
     - ``{"status": "no_outline", "reason": "parse_error", "error_type": "...",
       "line": 12}``. ``error_type`` הוא שם מחלקת החריגה — בלעדיו אי אפשר
       לאבחן למה קובץ שנראה תקין לא נותן מפה
