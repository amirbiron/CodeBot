...
.. list-table::
   ...
   * - ``codekeeper_get_note``
     - פתק **בודד** לפי ``note_id``: הגוף הנוכחי בדיוק כפי שהוא מאוחסן,
       הכותרת, הצבע, ``version`` — מספר הגוף הנוכחי — ואיפה הפתק יושב,
       בדיוק בארגומנטים שכלי הרשימה המתאים דורש. פתק של משתמש אחר, ופתק
       ריפו למי שאינו אדמין — ``not_found``. ראו :ref:`mcp-get-note`
...
ואיפה הפתק יושב, במוסכמה של פגיעת חיפוש: ``target`` ואיתו בדיוק
...
נמדד. גוף שאינו מחרוזת (אף כותב אינו שומר כזה) מדווח ``null`` — "לא ידוע",
לא "ריק". כך הזרימה ללוח גדול היא רשימה קטנה ואחריה ``codekeeper_get_note``.
השאילתה למסד אינה משתנה — הגוף עדיין נקרא ורק אינו נשלח; היטלה בגבול
המסד, כמו שהחיפוש עושה, היא שינוי נפרד. ``codekeeper_list_repo_notes`` נשאר
בלי הפרמטר: קובץ בריפו חסום ל-``MAX_NOTES_PER_REPO_FILE`` פתקים, והפער
הוא בלוחות.
...
לפניה. ``1`` הוא פתק שמעולם לא נדרס דרך השרת הזה: ההיסטוריה נכתבת רק
