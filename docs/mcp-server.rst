...
.. list-table::
   ...
   * - ערך
     - מתי
   * - ``outline``
     - הועבר ``outline`` אמיתי. גם קריאה שמעבירה גם ``lines`` (ונדחית
       כ-``outline_and_lines``) נספרת כאן — היא לא קראה תוכן
   * - ``query``
     - הועבר ``query`` — מופעים במקום תוכן, כלומר קריאה זולה מאותה משפחה
       כמו ``outline``. נבדק **לפני** ``lines``, ולכן גם קריאה שמעבירה את
       שניהם (ונדחית כ-``query_and_lines``) נספרת כאן, מאותו נימוק
   * - ``range``
     - הועבר ``lines``
   * - ``full``
...
.. note::
   ...
   ``codekeeper_search_code`` נושא פרמטר בשם ``query`` והוא **אינו** קריאת
   קובץ. מה שמונע ממנו להיכנס לעמודה הזו הוא שהקולבק מסתעף על רשימת הכלים
   ולא על שם הפרמטר — ``FILE_READ_TOOLS`` ב-``mcp_server/analytics.py``.
