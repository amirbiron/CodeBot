...
- **עלות פרסור אחד** היא ``_PARSE_RSS_PER_INPUT_BYTE`` כפול הקלט הגדול ביותר
  ...
  (‏``markdown-it-py`` הנעוץ) — הפרסר שכלי ה-Markdown של PR 5 יריץ — כלומר הוא
  מקדים-סידור: הכלי שמפרסר **היום** הוא ``codekeeper_docs_get_section``, שמריץ
  ``services.rst_parser`` על ``docs/*.rst`` בלבד, והוא זול בהרבה — קורפוס ה-RST
