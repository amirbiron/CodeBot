...
.. list-table::
   ...
   * - ``$mcp_error_message``
     - **נשלח**, כשהערך מחרוזת. הודעת החריגה של קריאה שנכשלה, שבלעדיה
       הדשבורד יודע שמשהו נדחה ולא איזה שדה. ראו את האזהרה מתחת לטבלה —
       ההודעה הזו עלולה לשאת קטע מהקלט שנדחה
   * - ``$mcp_intent``
     - **נשלח**, כשהערך מחרוזת. המשפט שהסוכן כותב על מה שהוא מנסה להשיג.
       ראו "לכידת הכוונה" למטה — הוא נוצר רק כי ``context`` דלוק
   ...
   * - ``$mcp_tool_name`` · ``$mcp_duration_ms`` · ``$mcp_is_error`` ·
       ``$mcp_error_type`` · ``$mcp_client_name`` · ``$mcp_intent_source`` ·
       ``$session_id`` · ``$mcp_listed_tool_names``
...
תחת אותו שם אינם המשפט הזה אלא מבנה שלם, והוא עובר שלם. זה נמדד ולא נוסח —
מילון תחת ``$mcp_error_message`` עבר גרסה קודמת של השער עם גוף קובץ בתוכו.
