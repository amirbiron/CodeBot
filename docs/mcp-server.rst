...
כלי רשימה סגורה: ``ok``, ``outline``, ``binary``, ``too_large``
ו-``no_outline``. ל-``error`` אין רשימה כזאת כאן **בכוונה** — הערכים
נגזרים מהכלי שסירב (``path_denied``, ``outline_and_lines``,
``page_too_large``, ``range_out_of_bounds`` ועוד רבים), ורשימה חלקית
שנראית סגורה גרועה מאין רשימה. ערך ``status`` חדש שיתווסף לקוד מתווסף
גם כאן.
...
.. list-table::
   ...
   * - ``ck_read_mode``
     - **נשלח**, כשהערך אחד משלושה: ``outline``, ``range`` או ``full``. מאפיין
       שהשרת מחשב בעצמו — ראו "מצב הקריאה" למטה
...
הוא אחת משלוש המילים האלה. השער אוכף את הקבוצה הסגורה — מאפיין משלנו אינו
