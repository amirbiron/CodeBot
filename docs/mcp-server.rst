...
מיוחד בשאילתה הוא תו. זה מה שמבדיל את הפרמטר הזה משני החיפושים האחרים:
``codekeeper_search_code`` הוא ``$text`` של מונגו ולכן מתאים למילים שלמות,
ו-``codekeeper_search_repo`` עובר ל-``git grep -E`` כשהשאילתה נראית כמו
רג'קס.

**ההתאמה אינה רגישה לרישיות**, כמו ``git grep -i`` ב-``codekeeper_search_repo``.
ההשוואה היא ב-``casefold``, אותה בחירה כמו ``symbol=`` באאוטליין.

**השאילתה אינה מקוצצת.** ``codekeeper_search_repo`` מריץ ``strip()`` על
השאילתה; כאן לא, כי חיפוש של הזחה (``"    return"``) הוא שימוש אמיתי וקיצוץ
היה משנה בשקט את מה שביקשו. הקיצוץ משמש רק כדי להכריע אם השאילתה ריקה.
