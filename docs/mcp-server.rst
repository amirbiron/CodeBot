...
- **עלות פרסור אחד** היא ``_PARSE_RSS_PER_INPUT_BYTE`` כפול הקלט הגדול ביותר
  ...
  בת תו אחד בכל שורה ל-90.2 (44.0MiB לפרסור אחד, מעל 35.2MiB שהנוסחה מקצה לחוט
  — ורק מריפו הדוקומנטציה שברשימת ההיתר; נמדד 2026-09-20 באותה שיטה). העלות לכל
  ...
  כלומר 141MiB לפרסור אחד, מה ששום רוחב מאגר אינו סופג. לצורה העוינת הכלי הוא תקרת טוקנים
  בפרסר עצמו, והיא מחוץ ל-PR הזה. ‏``scripts/measure_md_parse_cost.py`` מודד את
...
- **רצפה** (``_READ_POOL_FLOOR``, ‏2): כדי שקריאת מונגו תקועה אחת לא תשבית
  ...
  ש-CPython בחר ל-executor ברירת המחדל שלו; הסקירה (#3433) הורידה אותה.
