...
.. list-table::
   ...
   * - ``codekeeper_search_notes``
     - חיפוש פתק **לפי שם בלבד** בשלושת המקומות שפתק יכול לשבת בהם — קובץ,
       לוח, או קובץ בריפו משוקף. התאמה על חלק מהשם, בלי תלות ברישיות. כל
       פגיעה אומרת איפה הפתק יושב, בדיוק בארגומנטים שכלי הרשימה המתאים
       דורש. **אינו מחפש בתוכן הפתק** — ראו את ההערה מתחת לטבלה
...
.. note::
   ...
   **שני החיפושים אינם עובדים אותו דבר, ולא במקרה.**
   ...
   ``codekeeper_search_notes`` הוא רג'קס לא-מעוגן על ``title`` בלבד, ולכן
   **כן** תופס חלק ממילה — אבל רק בשם. חיפוש בגוף הפתק היה מחייב אינדקס
   טקסט שני על ``sticky_notes``, ומונגו מתיר **אינדקס טקסט אחד לכל אוסף**;
   זו החלטה חד-כיוונית שלא נשרפה כאן.
