...
- **עלות פרסור אחד** היא ``_PARSE_RSS_PER_INPUT_BYTE`` כפול הקלט הגדול ביותר
  ...
  שהתקציב יזוז עם התקרה. העלות לכל בית קלט היא **שיא** ה-RSS בזמן הפרסור
  — מה שכמה פרסורים במקביל מחזיקים יחד — נמדד על ``markdown-it-py`` הנעוץ
  דרך ``services.md_parser`` עצמו, והיא נגזרת מ**צפיפות הטוקנים** של המסמך
  ולא מגודלו: קורפוס ה-Markdown של הריפו עולה 24–27 בתים לכל בית קלט,
  המסמך הצפוף ביותר בו (כ-170 טוקנים ל-KB) עולה 72 — וזה הקבוע — וקובץ
  עוין של שורות-תבליט בודדות עולה כ-290, כלומר 142MB לפרסור אחד, מה ששום
  רוחב מאגר אינו סופג. הכלי קורא רק מריפואים משוקפים שברשימת ההיתר, ולכן
  הקבוע הוא המסמך הצפוף ביותר שהוא באמת מגיש; לצורה העוינת הכלי הוא תקרת
  טוקנים בפרסר עצמו, והיא מחוץ ל-PR הזה. ‏``scripts/measure_md_parse_cost.py``
  מודד את שלושת המספרים מחדש כשמחליפים גרסת פרסר.
- **רצפה** (``_READ_POOL_FLOOR``, ‏2): כדי שקריאת מונגו תקועה אחת לא תשבית
  ...
  זיכרון שהחשבון אמר שאין. **תקרה** (``_READ_POOL_CAP``, ‏32): מעליה עוד
  חוטים הם רק עוד פרסורים מקבילים תחת ה-GIL, ומעבר לתוכנית גדולה לא יהפוך
  10 ל-112 בלי שאיש החליט — זה אותו 32 ש-CPython בחר ל-executor ברירת
  המחדל שלו.
