...
.. list-table::
   ...
   * - ``codekeeper_docs_get_section``
     - סקשן בודד מקובץ תיעוד — RST או Markdown, לפי הריפו — במקום קובץ שלם.
       ...
       תת-סקשנים ושכנים לניווט. **קובץ שלם בקריאה אחת:** ``section`` עם
       הכותרת הראשונה בעץ (תת-הסקשנים כלולים כברירת מחדל), או, לאדמין,
       ``codekeeper_get_repo_file``. עדיף על ``codekeeper_get_repo_file``
       לקריאת תיעוד לפי סעיפים. איזה ריפו מגיש אילו נתיבים ובאיזה פורמט —
