...
.. warning::
   ...
   **ערך מעל התקרה נצמד בשקט, ואינו נדחה.** ``per_page=1000`` באאוטליין
   ...
   למה שנשלח. ``per_page=0`` נצמד ל-1 ולא לברירת המחדל, וערך שאינו מספר
   שלם חוזר לברירת המחדל.
...
הצלחה, ``error`` מתאר סירוב, ו-``error`` מגיע תמיד עם ``"ok": false``. ל-``status`` יש בתשובת
