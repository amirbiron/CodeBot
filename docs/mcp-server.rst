...
.. list-table::
   ...
   * - ``codekeeper_get_note_version``
     - תוכן של גרסה **קודמת** אחת. הגוף הנוכחי הוא ``codekeeper_get_note``,
       שגם אומר באיזה מספר הגוף הזה ייקרא כאן כשיידרס. השחזור עצמו הוא
...
על רשימות פתקים כלל:** ``OUTPUT_BYTE_BUDGET`` נמדד ונאכף רק על עמוד
אאוטליין, על חיפוש ועל ``query``; לוח שלם עבר מתחתיו, ומה שחסם היה הלקוח.
אכיפה שם היא שינוי נפרד. גוף פתק אחד חסום ב-``MAX_NOTE_CHARS`` תווים, ולכן
...
``id``, ``title``, ``color``, ``color_id``, ``updated_at`` ו-``content_bytes`` —
גודל הגוף המאוחסן ב**בתים** של UTF-8. היחידה נקובה בשם השדה כי בפתק עברי
...
הנוכחי ואומר ב-``version`` באיזה מספר הוא יופיע כאן כשיידרס — זה בדיוק
המספר שהצילום הבא נותן (``_next_note_version`` ב-``mcp_server/backend.py``
משרת את שניהם, ואינו משוכפל), ולכן ``codekeeper_get_note_version`` עם
המספר הזה יחזיר אחרי הדריסה בית-בית את מה ש-``codekeeper_get_note`` החזיר
לפניה; גוף ריק, שהצילום מדלג עליו, נושא ``null`` ולא מספר.
``1`` הוא פתק שמעולם לא נדרס דרך השרת הזה: ההיסטוריה נכתבת רק
במסלול ה-MCP (``update_note`` ב-``mcp_server/backend.py``), ועריכה מהוובאפ
אינה מוסיפה לה — ולכן גם אינה מזיזה את המספר. הגיזום אינו מזיז אותו אף
פעם, כי הגרסה החדשה ביותר אינה נגזמת לעולם.
