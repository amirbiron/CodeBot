...
.. list-table:: Operations & Feature Flags
   ...
   * - ``MCP_MAX_REQUEST_BYTES``
     - תקרת גודל (בתים) לגוף בקשה לשרת ה-MCP, לכל הנתיבים ולכל הכלים. גוף גדול יותר — לפי ``Content-Length`` או לפי מה שבאמת מגיע — נדחה ב-``413`` עם ``{"error": "body_too_large", "max_bytes": ...}`` **לפני** שהטרנספורט מפענח JSON. מינימום ``65536``; ערך שאינו מספר — ברירת המחדל, עם WARNING. ראו "גבולות הבקשה" ב-:doc:`mcp-server`.
     - לא
     - ``1048576``
     - ``2097152``
     - MCP
