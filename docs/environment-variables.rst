...
.. list-table:: Metrics & Predictive Controls
   ...
   * - ``METRICS_ROLLUP_SECONDS``
     ...
     - Bot/WebApp/Webserver
   * - ``METRICS_TTL_DAYS``
     - כמה ימים נשמר מדד ב-``service_metrics`` לפני מחיקה אוטומטית (אינדקס TTL על ``ts``). חייב לכסות את הטווח הארוך ביותר שדשבורד ה-Observability מציע ואת ``OBSERVABILITY_WARMUP_RANGES`` — אחרת התצוגות האלה יתרוקנו בלי שגיאה.
     - לא
     - ``30``
     - ``14``
     - Bot/WebApp/Webserver
