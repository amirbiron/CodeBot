...
.. list-table:: Operations & Feature Flags
   ...
   * - ``DISABLE_DAILY_REPORT``
     - מכבה את דוח הבוקר היומי. ריק = פעיל.
     - לא
     - (ריק)
     - ``true``
     - Bot
   * - ``DAILY_REPORT_HOUR_LOCAL``
     - שעת שליחת דוח הבוקר בפורמט ``HH:MM``, בשעון ישראל. ערך פגום חוזר ל-08:00 ונרשם כאזהרה.
     - לא
     - ``08:00``
     - ``07:30``
     - Bot
   * - ``DAILY_REPORT_CORRELATION_WINDOW_MINUTES``
     - רוחב חלון ההצלבה בין התראה לשאילתה איטית, בדקות. נחסם לטווח 1–120.
     - לא
     - ``5``
     - ``10``
     - Bot
