...
**תיאור:** פתקים דביקים. פתק שייך **או** לקובץ **או** ללוח — בדיוק אחד מהשניים.
...
.. list-table:: שדות sticky_notes (חלקי)
   ...
   * - ``file_id``
     - str
     - מותנה
     - הקובץ שאליו הפתק צמוד. ריק בפתקי לוח.
   * - ``board_id``
     - str
     - מותנה
     - הלוח שעליו הפתק יושב. ריק בפתקי קובץ.
   * - ``scope_id``
     - str
     - לא
...
.. important::
   **האילוץ "בדיוק אחד" נאכף ב-**\ ``sticky_notes_target.build_note_target``, שכל ארבעת מסלולי הכתיבה עוברים דרכו. אין ``$jsonSchema`` validator ברמת מונגו, כי אין בריפו תשתית migrations שתחזיק אותו — במקומו ``scripts/migrate_note_boards.py`` מדפיס דוח הפרות, והרצה חוזרת שלו היא הבדיקה.

**אינדקסים:** ``(user_id, file_id)``, ``(user_id, file_id, created_at)``, ``(user_id, scope_id)``, ``(user_id, board_id)``, ``(updated_at)``.
