...
.. list-table:: שדות code_snippets
   ...
   * - ``version_created_at``
     - datetime
     - לא
     - מתי **השורה הזאת** נכתבה. נכתב פעם אחת ביצירת מסמך הגרסה ואינו מתעדכן לעולם — וזו ההבחנה מ-``updated_at``, שהוא שדה של הקובץ ובר-שינוי (עדכון תיאור כותב אותו מחדש על הגרסה האחרונה). היסטוריית הגרסאות בוובאפ ו-``codekeeper_list_versions`` מציגים אותו. מסמך שנכתב לפני שהשדה נוסף נופל אחורה לחותמת הזמן שבתוך ה-``ObjectId`` שלו, שהיא אותו נתון בדיוק; הקריאה יושבת ב-``file_dates.version_created_at``. שחזור מגיבוי מקבל את **זמן השחזור** ולא את התאריך שבגיבוי, אחרת ההיסטוריה הייתה יוצאת מסדר
   * - ``description_set_at_version``
     - int
     - לא
     - מספר הגרסה שבה ה-``description`` נקבע או השתנה לאחרונה. גרסה חדשה עם אותו תיאור **יורשת** את הערך, וגרסה עם תיאור שונה מקבלת את מספרה שלה; עדכון תיאור בלי גרסה כותב את מספר הגרסה הנוכחית, וניקוי התיאור מסיר את השדה. הכלל יושב ב-``file_description.py`` שבשורש וכל מסלול שכותב גרסה מריץ אותו. ממנו נגזר בזמן קריאה ``description_age_versions`` שכלי ה-MCP מחזירים — ראו :ref:`mcp-description-age`. השדה **נעדר** ממסמכים שנכתבו לפניו ומקבצים שהמיגרציה לא הצליחה לתארך, ואז הגיל הוא ``null`` ולא אפס
   * - ``is_active``
     - bool
     - לא
     - האם הקובץ פעיל (ברירת מחדל: true). מחיקה רכה חלה על **כל** הגרסאות של הקובץ יחד, כי הקובץ הוא ``(user_id, file_name)`` והגרסאות הן ההיסטוריה שלו. מסכי הרשימה מוסרים לממשק את ה-``_id`` של הגרסה האחרונה בלבד, ולכן מחיקה שמסננת לפי מזהה משאירה את הקובץ חי — ראו ``file_deletion.py``
   * - ``deleted_at``
     - datetime
     - לא
