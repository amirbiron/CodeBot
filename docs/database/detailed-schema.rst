...
.. list-table:: שדות code_snippets
   ...
   * - ``updated_at``
     - datetime
     - לא
     - תאריך השינוי האחרון **בתוכן או בתיאור** של הקובץ. בקובץ שמעולם לא נערך הוא זהה ל-``created_at``. פעולות מטא-דאטה — נעיצה, מועדפים, מחיקה רכה ושחזור מהסל — **אינן** נוגעות בו; לכל אחת מהן יש שדה משלה (``pinned_at``, ``favorited_at``, ``deleted_at``)
   * - ``is_active``
     - bool
     - לא
