...
.. list-table:: שדות code_snippets
   ...
   * - ``updated_at``
     - datetime
     - לא
     - תאריך השינוי האחרון **בקובץ עצמו** — בתוכן, בתיאור או בשם. בקובץ שמעולם לא נערך הוא זהה ל-``created_at``. הקו הוא בין שדה של הקובץ לבין סימון עליו: נעיצה, מועדפים, מחיקה רכה ושחזור מהסל אינם משנים את הקובץ אלא רק תווית שעליו, ולכן **אינם** נוגעים ב-``updated_at``; לכל אחד מהם יש שדה משלו (``pinned_at``, ``favorited_at``, ``deleted_at``)
   * - ``is_active``
     - bool
     - לא
