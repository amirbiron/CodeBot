...
- feat: לחיצה על הכוונה המקוצצת פותחת חלון עם הטקסט המלא. התא הפך ל-``<button>`` אמיתי במקום ``span`` עם ``tabindex``, ולכן Enter ו-Space עובדים בלי קוד משלנו וקורא מסך מכריז "כפתור". ``Escape`` סוגר ומחזיר את הפוקוס לכפתור שפתח. הטקסט נכתב ב-``textContent`` — מסלול שני לאותו טקסט שסוכן חיצוני כתב, ולכן הוא נבדק בנפרד בדפדפן.
- feat: באנר "X שגיאות חדשות מאז הביקור האחרון" מעל טבלת הכשלים. הטבלה מציגה חלון נע של 30 יום, ולכן המספר הכולל יורד מעצמו כששגיאות ישנות נושרות ואינו יכול לשמש איתות; הבאנר משווה מול חותמת הזמן של החדשה ביותר שכבר נראתה. בביקור ראשון הוא שותק — אז הכול "חדש". ``localStorage`` חסום פשוט לא מציג אותו.
