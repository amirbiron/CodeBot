...
- fix: ה-ETag של עמוד הקובץ הוא הוולידטור **היחיד** שלו. הוא כולל את מצב המועדף והנעיצה, ואילו ``If-Modified-Since`` לבדו כבר אינו מייצר 304 בעמוד הזה: אין שדה שמתעד מתי מצב המועדף השתנה — ``favorited_at`` אומר מתי *סומן*, ובהסרת הסימון הוא מתאפס ו-``Last-Modified`` נסוג אחורה, כך שלקוח שהחזיק את הערך המאוחר היה מקבל 304 עם כוכב תקוע. ``Last-Modified`` ממשיך להישלח כמידע.
