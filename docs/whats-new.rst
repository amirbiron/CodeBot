...
2026-09-01
----------
- fix: שחזור ZIP לריפו מעלה את הקבצים לשורש, ולא בתוך תיקייה. ה-ZIP של גיבוי ריפו הוא ה-zipball של GitHub — שכל תוכנו תחת תיקייה בשם ``owner-repo-sha`` — עם ``metadata.json`` שנוסף **בשורש הארכיון**. הקובץ בשורש הוא בדיוק מה שמבטל את זיהוי תיקיית השורש, ולכן השחזור העלה לריפו גם את המניפסט וגם את כל התוכן רובד אחד עמוק מדי. המניפסט מוסר עכשיו **לפני** חישוב השורש, וההכרה בו נעשית לפי תוכן ולא לפי שם — ``metadata.json`` הוא שם קובץ לגיטימי בריפו, ודילוג לפי שם היה מוחק קובץ תוכן. אותו תיקון חל גם על ייבוא ZIP, ששכפל את אותו חישוב.
