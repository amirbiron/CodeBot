...
2026-08-27
----------
- fix: **פתק אינו חוצה עוד את גבול המסך במעבר בין מכשירים.** פתק שנוצר על מסך רחב מוקטן ומוצמד פנימה כשהוא מוצג על מסך צר — בסיבוב מכשיר, בכניסה מאותו מכשיר אחר, ובפתיחה מהתראת פוש. עד כה הסרגל העליון (ידית הגרירה) או הפינה הימנית-תחתונה (ידית שינוי הגודל) יצאו מהמסך, ולא הייתה דרך להחזיר את הפתק.
- fix: ההקטנה היא שכבת תצוגה בלבד ואינה נשמרת. הגודל שנקבע בגרירה מהפינה חוזר במלואו כשפותחים את הפתק שוב על המסך הרחב. רק גרירה מהפינה מעדכנת את הגודל השמור.
- fix: פתק ממוזער לא מאבד את גובהו. המסלול שהחזיר פתקים לתוך אזור התצוגה מדד את האלמנט, ובמצב ממוזער המדידה מחזירה את גובה הכותרת בלבד — הגובה השמור נדרס לרצפה.
- fix: פתק צף לא מתכווץ עוד בכל טעינת עמוד. אנימציית ההופעה של הפתק מתחילה ב-``scale(.9)``, והמסלול הזה מדד את הפתק בזמן שהיא רצה וכתב את המידה המוקטנת בחזרה — כלומר גרירה מיד אחרי טעינה שמרה פתק קטן יותר. נמדד בכרומיום.
- fix: שינוי גבולות מסך מריץ חישוב מחדש גם לפתקי לוח, לפתקי ריפו ולפתקים מעוגנים. קודם הוא דילג עליהם, כי הוא טיפל בפתקים צפים בלבד.
- docs: עודכנו :doc:`/user/sticky_notes` (ההתנהגות הנראית) ו-:doc:`/dev/sticky_notes_extending` (ההפרדה בין כוונה לתצוגה, ולמה ההקטנה חייבת להישאר מחוץ למה שנשמר).
