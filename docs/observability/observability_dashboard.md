...
3. **API מתועד** – מסלולי `GET` ו-`POST` תחת `/api/observability/` שניתנים לצריכה אוטומטית ע"י Grafana, Slack או סקריפטים. סעיף ה-API Reference שלמטה מתעד את המסלולים שהמסך הזה צורך. תחת אותה קידומת יש גם משפחות מסלולים שאינן מתועדות כאן — תגיות, coverage, drills וסיפורי Incident.
