...
.. list-table::
   ...
   * - סיבה
     - מתי
   * - ``owner_missing``
     - השאילתה אינה מצהירה על ``user_id`` (ברמה העליונה או בתוך ``$and``; באגרגציה — בשלב ה-``$match`` הראשון, היחיד שמגביל את כל מה שאחריו). ``$or`` לבדו אינו מגביל, ולכן אינו נספר.
   * - ``owner_mismatch``
     - ``user_id`` שאינו ברשימה מופיע בשאילתה, בכל עומק — כולל ``$or`` שמערב אותך ואחרים.
...
הערכים חיים ב-``slow_queries_log`` בלבד ונגישים דרך ה-API של האדמין. שורת הלוג
``slow_query_detected`` ממשיכה לשאת את השלד — הלוג עוזב לספק, ה-DB לא. ותשובת
ה-explain מנרמלת את השאילתה בעצמה, ולכן הדוח שמודבק ל-AI מכיל את השלד גם כשהניתוח
רץ על הערכים האמיתיים.
