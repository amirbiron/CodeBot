...
הפרופיילר מיועד ל-**Admin** בלבד, ו**סשן אדמין ב-WebApp נדרש תמיד**. ``_profiler_is_authorized`` (``webapp/app.py``) מסתיים בבדיקת אדמין בכל מסלול, ולכן:

- אין גישה בלי סשן אדמין — גם עם ``X-Profiler-Token`` תקין.
- כשיש סשן אדמין, הטוקן אינו נבדק כלל.
- ``PROFILER_ALLOWED_IPS``, אם הוגדר, חל **גם** על אדמין מחובר.

כלומר ``PROFILER_AUTH_TOKEN`` אינו פותח גישה ואינו חוסם אותה. ההערה בקוד מתארת כוונה אחרת ("אם אין token ואין allowlist — נדרוש אדמין"), וההפרש בין הכוונה להתנהגות הוא ממצא פתוח — לא משהו שהתיעוד מרכך.
...
הגישה מתבססת בפועל על הרשאת Admin ב-WebApp Session בכל מקרה — עם טוקן ובלעדיו (ראו `קהל יעד`_).
...
.. code-block:: bash
   ...
   curl -H "X-Profiler-Token: $TOKEN" \
        "https://your-app.com/api/profiler/slow-queries?limit=20&collection=code_snippets&sort=timestamp&dir=desc"
...
**וגם ערכי המבנה: ``$limit``, ``$skip`` וכיווני ``$sort``.** אלה אינם נתוני משתמש
אלא צורת השאילתה, והם ניתנים לאימות מלא בלי שום רשימה — שלם חיובי, שלם אי-שלילי,
או שדה ← ``1``/``-1``/``$meta``. הם נשמרים אמיתיים כי בלעדיהם הניתוח מתאר שאילתה
...
.. list-table::
   ...
   * - Token
     - ``PROFILER_AUTH_TOKEN``
     - טוקן נשלח ב-Header ``X-Profiler-Token``. אינו מספיק לבדו, ואינו נבדק כשיש סשן אדמין
   * - IP Allowlist
...
.. note::
   ...
   המיון בעמודות, אבל האינדקס עדיין לא נוצר: המיון תמיד מלווה בטווח על
   ``timestamp`` (חלון הזמן), וטווח בתחילית שובר את סדר המיון — כלל ה-ESR.
   כלומר גם אינדקס כזה לא היה מונע מיון בזיכרון. ארבעת שדות המיון האפשריים
   היו דורשים ארבעה אינדקסים שאף אחד מהם לא עוזר, ולכן המיון נשאר בזיכרון —
   נמדד ב-``executionStats``: כ-59KB במיון לפי משך ו-40KB לפי זמן, מול תקרה
   של 32MB, בלי spill. ה-TTL של שבעה ימים והסף של שנייה הם מה שחוסם את הגידול.
