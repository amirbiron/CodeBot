...
.. list-table::
   ...
   * - ``unsupported_type:<טיפוס>``
     - ערך שאינו שורד מסע הלוך-ושוב כ-JSON: ``ObjectId``, ``Decimal128``, ``bytes`` — **וגם ``datetime``**, ולכן כל שאילתה עם טווח תאריכים נשמרת בלי ערכים. ההגדרה אינה רשימת טיפוסים אלא הסיבוב עצמו (``json.dumps`` ← ``json.loads``), כך שטיפוס חדש נדחה בלי שאיש יעדכן רשימה. שמירה כמחרוזת הייתה נראית אמיתית ומריצה שאילתה אחרת — ומחזירה אפס תוצאות בלי שאיש יידע.
   * - ``unsupported_number``
     - ``inf``, ``-inf`` או ``NaN``. ``json.dumps`` פולט עליהם ``Infinity``/``NaN``, שאינם JSON תקני — ו-``JSON.parse`` בדפדפן זורק עליהם, כלומר הדשבורד היה נשבר על הרשומה.
   * - ``malformed``
