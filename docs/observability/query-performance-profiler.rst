...
.. list-table::
   ...
   * - ``owner_mismatch``
     - ``user_id`` שאינו ברשימה מופיע בשאילתה — **בכל שלב ובכל עומק**, לא רק בסינון. מזהה שיושב ב-``$set`` או ב-``$lookup.let`` נתפס בדיוק כמו מזהה ב-``$match``.
   ...
   * - ``unsupported_type:<טיפוס>``
     - ערך שאינו שורד את המסע חזרה כ-JSON: ``ObjectId``, ``datetime``, ``Decimal128``, ``bytes``. שמירתו כמחרוזת הייתה נראית אמיתית ומריצה שאילתה אחרת — ומחזירה אפס תוצאות בלי שאיש יידע.
   * - ``too_large``
...
**איפה הערכים חיים.** ברשומה ב-``slow_queries_log``, תחת ה-TTL של שבעה ימים,
ובנוסף ב-buffer שבזיכרון התהליך — החסום ב-``PROFILER_MAX_BUFFER_SIZE`` ומת עם
התהליך. **לא בלוגים:** שורת ``slow_query_detected`` ממשיכה לשאת את השלד, כי הלוג
עוזב לספק וה-DB לא. שני מסלולי הקריאה, מה-DB ומהזיכרון, בודקים את הרשימה העדכנית,
ולכן הסרת משתמש ממנה מסתירה את הערכים מיד בשניהם. ותשובת ה-explain מנרמלת את
השאילתה בעצמה, ולכן הדוח שמודבק ל-AI מכיל את השלד גם כשהניתוח רץ על הערכים
האמיתיים.
