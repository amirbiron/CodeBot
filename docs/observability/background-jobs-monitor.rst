...
**תקרת זמן על הפיוס כולו.** לקוח המוטור שהפיוס רץ עליו (``_get_scheduler_motor_db``
ב-``main.py``) נבנה בלי ``socketTimeoutMS``, ובלי דדליין משלו קריאה שנתקעה על
הסוקט הייתה תוקעת ג'וב חד-פעמי לנצח — בשקט. ‏``reconcile_job_callback`` עוטף את
הפיוס ב-``asyncio.wait_for`` עם ``RECONCILE_TIMEOUT_SECS``, וכל תוצאה — דילוג,
חיתוך, כשל, או "לא נמצא כלום" — נרשמת בלוג בשם אירוע משלה (ראו :doc:`events_catalog`).

**הלוגים של ההרצה נכתבים בתוספת.** הפיוס דוחף שורת לוג להרצה שסגר; כדי שהשורה
הזו תשרוד גם אם ההרצה עוד חיה ותכתוב את סופה, ``_persist_run`` דוחף רק את
השורות שטרם נכתבו (``$push`` עם ``$slice``) ואינו מחליף את המערך כולו.
