...
=========================
:summary: chatops.ratelimit מרכז את שכבת ההגנה הקלה נגד ספאם לפקודות רגישות במערכת ה-ChatOps (למשל `/deploy`, `/restart`, `/secrets`). במקום ליישם Redis/DB עבור כל פקודה, אנחנו שומרים חותמת זמן אחרונה בזיכרון ה-process ומוודאים שהמשתמש מחכה כמה שניות בין הרצות.
