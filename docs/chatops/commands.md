---
summary: 'להלן מבנה אחיד לכל פקודה: מתי להשתמש, פרמטרים, הרשאות, מה לחפש בפלט, ודוגמה קצרה אם יש ערך מוסף.'
---
...
- מה לחפש בפלט:
  ...
  - 👻 במקום ❌ — הרצה שנסגרה בפיוס הרצות יתומות (`failure_reason: orphaned`), לא ג'וב שנפל בקוד שלו. אותו סימון ב-`failed` ובהיסטוריה של `<job_id>`.
  - משך ההרצה בהיסטוריה מוצג כמו האינטרוול: שניות, דקות, שעות או ימים
...
## /ban
- מתי להשתמש: לחסום משתמש מלהשתמש בבוט (למשל בגלל ספאם או שימוש לרעה)
- פרמטרים: `<user_id> [reason]`
  - `user_id` (חובה): מזהה מספרי של משתמש טלגרם. **לא תומך ב-@username** — רק מזהה מספרי
  - `reason` (אופציונלי): סיבת החסימה לתיעוד
- הרשאות: מנהלים בלבד (`ADMIN_USER_IDS`)
- מה לחפש בפלט:
  - הצלחה: "✅ [user_id] נחסם" עם הסיבה אם צוינה
  - כישלון: "❌ החסימה נכשלה" אם בסיס הנתונים לא זמין
  - "❌ אי אפשר לחסום אדמין" אם ניסיון לחסום מנהל
- הערות: אדמינים לא ניתן לחסום. החסימה נכנסת לתוקף מיד ונאכפת בשער הכניסה לפני כל handler
- דוגמה:
```
/ban 123456789 ספאם
/ban 987654321
...
## /unban
- מתי להשתמש: לשחרר משתמש שנחסם בעבר
- פרמטרים: `<user_id>` (מזהה מספרי בלבד)
- הרשאות: מנהלים בלבד (`ADMIN_USER_IDS`)
- מה לחפש בפלט:
  - הצלחה: "✅ [user_id] שוחרר"
  - "ℹ️ [user_id] לא היה חסום (או שבסיס הנתונים לא זמין)" — **שני המקרים מחזירים את אותה הודעה.** אם השחרור חשוב, ודאו עם `/blocked` שהמשתמש באמת ירד מהרשימה, ולא הסתפקו בהודעה
  - "⚠️ [user_id] חסום דרך BLOCKED_USER_IDS" — משתמש שחסום דרך משתנה הסביבה BLOCKED_USER_IDS לא ניתן לשחרר דרך הפקודה
- הערות: לא ניתן לשחרר משתמשים שחסומים דרך `BLOCKED_USER_IDS` (רשת ביטחון ברמת ENV)
- דוגמה:
```
/unban 123456789
```
...
## /blocked
- מתי להשתמש: לראות את רשימת המשתמשים החסומים כרגע
- פרמטרים: אין
- הרשאות: מנהלים בלבד (`ADMIN_USER_IDS`)
- מה לחפש בפלט:
  - רשימה של משתמשים חסומים דרך `BLOCKED_USER_IDS` (רשת ביטחון — לא ניתן לשחרר מהבוט)
  - רשימה של משתמשים חסומים דרך בסיס הנתונים (ניתנים לשחרור דרך `/unban`)
  - לכל רשומה: מזהה המשתמש, תאריך החסימה, וסיבה אם צוינה
- דוגמה:
```
/blocked
```
