...
**ניסיונות חוזרים** – אין ניסיון חוזר על ``4xx``. תשובת ``4xx`` היא הכרעה
של שירות הפוש או של ה-Worker, וחזרה עליה תיתן את אותה תשובה. ניסיון חוזר
מתבצע רק על ``5xx`` ועל timeout.
