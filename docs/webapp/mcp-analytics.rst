...
.. list-table::
   ...
   * - ``POSTHOG_PROJECT_ID``
     - מזהה הפרויקט המספרי, לבניית נתיב ה-API
   * - ``POSTHOG_HOST``
...
**שלושת המספרים תלויים זה בזה.** התקציב חייב להיות גדול מהמקרה הגרוע של קריאה
בודדת (``timeout × max_attempts`` ועוד ה-backoff); אחרת הוא חותך קריאה שעוד
עשויה להצליח, ומשאיר אחריו עבודה שרצה בלי שאיש ממתין לה. הקבועים יושבים בראש
``services/mcp_analytics_service.py``, והיחס ביניהם נאכף בבדיקה ולא בהערה.
...
``tests/test_mcp_analytics_service.py`` ו-``tests/test_admin_mcp_page.py``.
בדיקות העמוד עוברות דרך בקשת HTTP ובודקות את ה-DOM המרונדר, ולא קוראות
לפונקציות ישירות.
