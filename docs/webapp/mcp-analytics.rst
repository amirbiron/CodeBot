...
.. _mcp-analytics-fresh:

"X שגיאות חדשות מאז הביקור האחרון"
------------------------------------

הטבלה מציגה חלון **נע** של 30 יום, ולכן המספר הכולל יורד מעצמו כששגיאות ישנות
נושרות — גם כשנוספו חדשות. כלומר הוא אינו יכול לשמש איתות.
...
.. list-table::
   ...
   * - ביקור ראשון (אין ערך שמור)
     - הבאנר **שותק**. הכול היה "חדש", ומספר כזה אינו אומר דבר. הביקור כן
       נרשם, אחרת גם הבא אחריו היה "ראשון"
   * - ``localStorage`` חסום (גלישה פרטית, הגדרות)
     - הבאנר פשוט לא מופיע. הקריאה והכתיבה עטופות, וכשל שלהן אינו נוגע בשאר
       העמוד — זו נוחות, לא נתון
   * - ההשוואה
     - על ``data-at`` שנושא את הערך הגולמי מ-PostHog, ולא על המחרוזת המוצגת:
       התצוגה מעוגלת לדקה ומנוסחת בעברית
   * - הטבלה נחתכה בתקרה, וכל השורות שהוצגו חדשות
     - הניסוח הופך ל"לפחות X". הבאנר סופר שורות ב-DOM, והשאילתה מוגבלת
       ל-``TOOL_FAILURES_LIMIT`` — ולכן המספר הוא **רצפה** ולא ספירה

.. note::

   הערך הוא לכל דפדפן בנפרד, לא לכל משתמש. זו החלטה מודעת: הספירה היא עזר
   תצוגה, והיא לא שווה אוסף חדש במסד או מסלול כתיבה נוסף. מאותה סיבה אין
   ספירה אמיתית בצד השרת כשהטבלה נחתכת — היא הייתה דורשת לשלוח לשרת את
   חותמת הזמן ששמורה בדפדפן ולקבל ספירה בחזרה, בקשה נוספת עבור שורת טקסט.
