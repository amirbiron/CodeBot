...
יצירת גיבוי מלא, שחזור, העלאה ל-Google Drive, ניהול הגיבויים הקיימים, וייבוא ZIP חיצוני.
...
‏``github_repo_zip`` אינו ארכיון שהבוט בונה בעצמו. הוא ה-zipball הרשמי של GitHub — שכל תוכנו יושב תחת תיקייה אחת בשם ``owner-repo-sha`` — ואליו מתווסף ``metadata.json`` **בשורש הארכיון**::
   ...
   metadata.json                       ← מניפסט הגיבוי, אינו חלק מהריפו
   owner-repo-6dfaac9/README.md
   owner-repo-6dfaac9/src/main.py

**המניפסט הוא נלווה, לא תוכן.** כל מסלול שפורס את הארכיון חייב להסיר אותו **לפני** חישוב תיקיית השורש, ולא רק מרשימת הקבצים לפריסה. הסיבה מכנית: ``detect_zip_common_root`` מחזירה ``None`` ברגע שיש קובץ כלשהו בשורש — וזה הכלל הנכון שלה — ולכן מניפסט שנשאר במקומו מבטל את זיהוי ``owner-repo-sha``, והשחזור מעלה את הריפו כולו רובד אחד עמוק מדי ואת המניפסט לצידו. ``zip_members_and_root`` ב-``handlers/documents.py`` מבצעת את שני הצעדים יחד, ושני המסלולים שפורסים ZIP קוראים לה.

**וההסרה נעשית לפי תוכן, לא לפי שם.** ``metadata.json`` הוא שם קובץ לגיטימי בריפו אמיתי, ודילוג לפי שם היה מוחק קובץ תוכן. קובץ נחשב מניפסט רק אם הוא JSON תקין שיש בו ``backup_id`` ועוד שדה מזהה (``backup_type`` או ``created_by``).
