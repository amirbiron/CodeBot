...
=================================
:summary: מנוע הרפקטורינג מאפשר שינוי מבנה קוד בצורה בטוחה עם אימות לפני ואחרי.
...
- לחיצה עליו מייצרת Gist מרובה-קבצים עם תיאור שמציין את סוג הרפקטורינג ואת מספר הקבצים.
- ה-Gist נוצר תחת חשבון ה-GitHub האישי של המשתמש. האינטגרציה מתקבלת מ-``integrations.resolve_gist_for_user(user_id)``, ולא מטוקן גלובלי — כך ה-Gist שייך למי שביקש אותו.
- ``GITHUB_TOKEN`` (משתנה הסביבה) אינו משמש כאן. הפיצ'ר זמין רק למשתמש שחיבר את חשבון ה-GitHub שלו דרך תפריט ראשי ← 🔧 GitHub; אחרת מוצגת הודעה עם הנחיית חיבור והמערכת אינה מבצעת קריאה ל-GitHub.
- הקריאה ל-``resolve_gist_for_user`` ול-``create_gist_multi`` חוסמת (DB ורשת), ולכן היא רצה דרך ``asyncio.to_thread`` כדי לא לתקוע את ה-event loop של הבוט.
